...
> 界面为原生面板。**旧版界面已在 `2.0.0` 中整体移除**，本版不再提供任何回退入口；如确需旧界面，请安装最后一个提供它的版本 `1.41.1`（设置 → 关于 里也保留了该下载入口，计划在 `2.3.0` 移除）。

### 2.0.0 有什么变化

- **旧版界面整体移除**：工具栏旧菜单、旧设置页与内嵌 iframe 界面都不再提供，也不再有「切回旧界面」的开关。
- **浏览器扩展、挂件、网页版重建**：三者改用同一套界面；扩展与网页版首次使用需在「连接配置」中填写思源 API 地址与 Token（挂件与思源同源，无需填写）。
