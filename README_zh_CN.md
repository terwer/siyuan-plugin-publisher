...
> 最近的Bug修复 `1.14.1`
  - 本地更新新增了特性，notion支持根页面搜索。同时修复了非Github平台发布错误的问题以及修复默认导入未导入WordPress的问题。
  - **如果您之前出现无法发布的问题，那么本次更新应该能解决您的问题，建议所有用户升级。**

> 最近的新特性版本：🎉 发布工具新版本 `1.14.0` 发布

  - 本次更新新增多个平台，包括简书、掘金以及 Hugo、Jekyll、Vuepress、Vuepress2、Vitepress，后面几个平台同时兼容 `Github` 和 `Gitlab`
...
> 平台限制说明
  ...
  - **掘金**：**掘金每次发布文章都需要审核** ，因此发布文章之后马上预览可能会404，此时可修改链接的 `/post` 为 `/spost` 临时查看，或者耐心等待审核通过。
