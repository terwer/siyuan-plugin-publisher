...
export default {
  ...
  "blog.top-data-tip":
    "温馨提示：1. 请保证思源笔记启动并且打开伺服，默认伺服地址：http://127.0.0.1:6806。 2. 0.1.0+ 以后，您可以在本页面底部点击【修改思源API地址】来修改默认配置，注意：此处修改会覆盖启动设置的值。 3. 为了获得更好的使用体验，建议使用【新窗口打开】首页。",
  "blog.top-data-tip.siyuan":
    "温馨提示：当前处于挂件模式，从 0.1.0 版本开始，无需在所有页面引用挂件了，只需要在父级页面引用一个挂件即可。具体规则如下：1. 如果检测到没有子文档，会兼容 0.0.3 版本以前的方式，展示当前文档的发布页面。2. 如果检测到有子文档，会以列表加分页的方式展示所有子文档列表。可单独选择某个子文档进行发布操作。",
