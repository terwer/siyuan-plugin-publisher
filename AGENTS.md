...
### 0. 用户配置是资产，禁止删除（最高优先级，不可违反）

- **宿主里的平台账号配置属于用户资产，一律禁止删除**；验证过程中新建的账号同样**保留**。
- 需要减少账号数量时，**先向用户说明并取得明确同意**，由用户决定。
- 平台端的**测试文章**属于验证产物，可按需清理；**账号配置不在其列**。
- 历史上删除过的账号须**尽力找回**（先查快照 / 备份 / 记录，能重建则重建），并如实报告恢复情况。
- 取用宿主数据时遵循同一取向：**只增不删**，宁可多留一个未授权账号，也不移除用户已有的任何配置。

### 1. 插件宿主容器约束

- **插件挂载在思源宿主 DOM 的 popup/面板区域内**，不是独立网页，不是整个浏览器窗口。
...
- **小心求证**：每次改动以单测、构建、宿主手验、checklist 为证据链。
...
- **优化目标排序**：**最大扩展性** > **可用性/用户体验** > **维护成本**；允许结构性 break change，但不牺牲上述行为保留。
...
### 根因（一句话）
旧版插件写入**全小写**属性 key（`custom-fs-localsystem-yaml` → 推导 `fs_localsystem`），而配置 `DynamicConfig.platformKey` 是**混合大小写**（`fs_LocalSystem`）；原 `getDynCfgByKey` 只做全等 → 返回 `null` → 显示「已删除」、图标回退 `iconOTRemove`。V1（`Admin.vue`）与 V2（`ArticleManage.vue`）共用的 `useArticleManage` / `ArticleManageList` 均受影响（仅显示与「点击平台单发」的 key 解析，不碰发布数据）。
...
- **每站验证必须同时覆盖「帮助引导与文档」环节，禁止遗漏**（SOP 第三节，`.internal-docs/platform-verification-sop.md`）：① 平台独立 help 配置（`src/helpConfigs/pages/platform-config/<platform>.ts`，须含 `helpUrl`+`summary`+`fields`+`faq`+`tour`，缺则补；由 `registry.spec.ts` 的 `verifiedConfigs` 与 `verifiedPlatformRows.ts` 冻结行强制约束）；② `docs/draft/platforms/<platform>.md` 文档草稿（占位 helpUrl 顶部标 `TODO：待替换真实帮助文档链接`）；③ 宿主验证：该平台配置页 HelpPanel/TourGuide 正常展示。此三项与五格同等计入通过/失败，一并写回 checklist SSOT。
...
- **Cookie 授权平台 → Cfg 标准流程（固化规则，与用户对齐，勿颠倒/跳过）**：对 Cookie 授权类网页平台（**微信公众号 / 掘金 / 知乎 / 简书 / CSDN / 语雀网页版 / Halo网页版** 等，配置页含「去授权/去登录 + 自动读取 Cookie」），在宿主做 Cfg 严格按此顺序：
  ...
  > 第 5 步（检查并填写其他必需字段）最易漏；顺序颠倒/跳过任一步都会导致 Cookie 读不到或账号停留「待验证/配置错误」。此规则由多个平台验证沉淀，必须遵守。
...
- **构建链**：插件本体由 `vite.config.ts` 构建到 `dist/`，命令 `pnpm build`（含 `vue-tsc`）；调试/watch 用 `pnpm dev`；软链到思源用 `pnpm makeLink`。**只有这一套构建链，不存在第二套界面构建。**
- **代码结构（无版本号命名）**：界面代码集中在 `src/ui/**` —— `components/` 为原生实现、`components/bridge/**` 为复用的共享表单组件（发布表单、平台配置表单、文章管理表格、AI 聊天）、`composables/**`、`assets/**`；插件宿主运行时在 `siyuan/host/**`；四个 web 产物的壳在 `src/webapp/**`。（`mv2` 指 Firefox Manifest V2，与命名清理无关。）
- **四个 web 产物**：挂件 / 浏览器扩展（chrome、edge、firefox）/ nginx / vercel 由同一份通用壳构建 —— `src/webapp/**` + `vite.webapp.config.ts`（按 `BUILD_TYPE` 落 `widget` / `extension/<type>` / `nginx` / `vercel`）。壳通过 `hostAdapter` 判定宿主（扩展 / 挂件 / 网页版）决定文档来源与是否显示「思源连接配置」。构建脚本传的 `--outDir` 必须是绝对路径（Vite 会把相对路径按 config 的 `root` 解析）。
...
- 平台配置校验失败通过 `SypErrorDetailsPanel`（及行内摘要）展示 `errorMessage`。
...
- **`refactor-form-upload-transport`**：归档前须在宿主手验 checklist **#27、#28 的 Img**（见 checklist 修订记录）。
