...
- **代码结构（无 V2 命名）**：界面代码集中在 `src/ui/**` —— `components/` 为原生实现、`components/bridge/**` 为复用的共享表单组件（发布表单、平台配置表单、文章管理表格、AI 聊天）、`composables/**`、`assets/**`；插件宿主运行时在 `siyuan/host/**`；四个 web 产物的壳在 `src/webapp/**`。（`mv2` 指 Firefox Manifest V2，与命名清理无关。）
