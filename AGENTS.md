...
- **Cookie 授权平台 → V2C 标准流程（固化规则，勿颠倒顺序）**：对 Cookie 授权类网页平台（**微信公众号 / 掘金 / 知乎 / 简书 / CSDN / 语雀网页版 / Halo网页版** 等，配置页含「去授权/去登录 + 自动读取 Cookie」），在 V2 宿主做 V2C 必须严格按此顺序：
  1. 配置页点「**去授权 / 去登录**」→ 插件打开该平台登录窗口（扫码/账号登录）。
  2. 用户用平台账号登录成功。
  3. **关闭该登录窗口**——此步触发插件把登录态 **Cookie 保存**到本地（关键一步，勿跳过、勿直接关宿主页、勿在未关窗时点别的）。
  4. 回配置页点「**2 自动读取 Cookie**」→ 把刚保存的 Cookie 读进该账号配置。
  5. 点「**验证**」→ `validatePublish` 通过 → 账号状态「已授权/运行中」。
  > 试过 3~4 个平台仍记不住，原因就是这条流程没固化；**顺序颠倒会导致 Cookie 读不到**，账号停留在「待验证 / 配置错误」。此规则由多个平台验证沉淀，必须遵守。
