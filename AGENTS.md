...
- **宿主里的平台账号配置（含验证用的账号）属于用户资产，一律禁止删除**。验证需要新建账号时，**保留**，不要"验完就删"。
- **禁止**以"复原账号数基线""清理临时账号""验证规范"等任何理由删除任何账号。**没有临时账号这回事**——这是用户的真实工作空间，不是临时测试环境。
- 需要减少账号数量、或确需删除时，**必须先向用户说明并取得明确同意**，由用户决定。
- 判断平台/功能时**只增不删**：宁可多留一个未授权账号，也不可删掉用户的任何配置。
- 若历史上已删除过账号，**必须尽力找回**（先查快照 / 备份 / 记录，能重建则重建），并如实向用户报告恢复情况。
