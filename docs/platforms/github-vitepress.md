...
| 图床 | 选「当前平台」：图片上传到**文章所在目录**的 `images` 子目录（规则 `[docpath]/images`，如 `docs/images/<图片名>`），文章引用相对路径 `./images/<图片名>` |
...
- 文章内引用 `imageLinkPath = ./images` 生成**相对路径** `./images/<图片名>`，随页面一起输出。
...
![填写图片存储目录与访问链接](/images/shots/path-vitepress.webp)
...
VitePress 推荐用**相对路径**引用图片，因此链接写 `./images`。一般不需要改，改了请保证两个值**对应**。
