...
通过 **GitHub API** 将文章发布到 **Hugo** 静态博客仓库。
...
- 文章的最终 URL（permalink）由**所在 section（content 下的目录）+ slug** 决定。默认 `[slug].md` 文件名 + 开启「YAML 永久链接」时，插件会在 front matter 写入 `url`，取值**固定为 `/post/<slug>.html`**（不读取「文章预览规则」，只是两者默认恰好一致），保证查看链接可打开；若你改了「文章预览规则」，需自行确认站点实际地址是否仍是 `/post/<slug>.html`。
...
| 字段 | 默认值 |
|---|---|
| 图片存储目录 | `static/images` |
| 图片访问链接 | `images` |

Hugo 的图片放在 `static` 下，构建后会原样复制到站点根目录。一般不需要改，改了请保证两个值**对应**。
...
- **查看链接打不开**：开启「YAML 永久链接」时，front matter 的 `url` 会把文章地址固定为 `/post/<slug>.html`，确认「文章预览规则」与此一致；若关闭，则地址由 content 下目录与 slug 决定，需对应调整预览规则。另外请确认博客使用的主题/发布脚本会对仓库变更执行 `hugo` 构建（如 GitHub Actions），否则新文章不会出现在线上站点。
