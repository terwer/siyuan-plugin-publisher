...
5. 注册适配器
   ...
   YAML适配器(不一定有)
6. 开启配置页面
   在 src/ui/components/settings/bridge/bridgeRegistry.ts 注册该平台的配置组件
7. 修改图片上传逻辑，可选
...
| 目标 | 命令 | 输出 |
|---|---|---|
| 插件本体 | `pnpm build`（含 `vue-tsc` 类型检查） | `dist/` |
| 单独构建插件 lib | `pnpm pluginBuild` | `dist/` |
| 思源挂件 | `pnpm widgetBuild` | `widget/` |
| 浏览器扩展（Chrome） | `pnpm extBuild` | `extension/chrome/` |
| 浏览器扩展（Firefox） | `pnpm extBuild -t firefox` | `extension/firefox/` |
| nginx 部署 | `pnpm nginxBuild` | `nginx/` |
| vercel 部署 | `pnpm vercelBuild` | `vercel/` |
