> **历史文档（2026-09-20 备注）**：本文描述的是「旧界面与现界面并存、由 useV2UI 分流」时期的接线。旧界面已在该日整体移除 —— useV2UI 开关与旧菜单、src/pages/**、src/routes/**、vite.v1*.config.ts 均不存在；插件本体由 vite.v2.config.ts 构建到 dist/，挂件/浏览器扩展/nginx/vercel 由通用壳（src/webapp/** + vite.webapp.config.ts）构建。下文保留当时事实，勿作为现行依据。
...
- **视图层**：界面有自己的统一工作壳 `V2App + UnifiedWorkspaceShell`
- **业务层**：界面 **没有重写核心发布业务**，而是继续复用老的 `usePublish / usePublishConfig / useSiyuanApi`
- **设置表单层**：平台配置页大部分仍然是 **壳 + 旧表单组件桥接**
...
1. 界面和旧 quick publish 在发布前预处理是否一致
...
| 能力 | 当前状态 | 代码锚点 |
|---|---|---|
| 顶栏主入口快速发布 | 已切到原生 DOM | `siyuan/topbar.ts:67-90` |
| 发布设置壳入口 | 已切到原生 DOM | `siyuan/invoke/widgetInvoke.ts:109-120` |
| 统一工作壳 | 已存在 | `src/components/v2/V2App.vue` |
| 账号列表 | 原生实现 | `src/components/v2/settings/V2AccountList.vue` |
| 账号选择 | 原生实现 | `src/components/v2/settings/V2PlatformSelect.vue` |
| 图床设置 | 原生实现 | `src/components/v2/settings/V2PicBedSettings.vue` |
| 偏好设置 | 原生实现 | `src/components/v2/settings/V2PreferenceSettings.vue` |
| 快速发布卡片视图 | 原生实现 | `src/components/v2/publish/V2PlatformCard.vue` |

## 5.2 仍然是壳 + 旧表单桥接

| 能力 | 当前状态 | 代码锚点 |
|---|---|---|
| 平台配置表单 | 外壳承载，内部复用旧表单组件 | `src/components/v2/settings/V2PlatformConfigBridge.vue` |
...
- 偏好配置文件：`/data/storage/syp/publish-preference-cfg.json`
  ...
  - Vue 响应式读法：`src/stores/usePreferenceSettingStore.ts:21-89`
...
2. **做原生表单**：替换 `V2PlatformConfigBridge` 的桥接分支
...
而界面 quick publish 直接只发当前平台：

- `src/composables/v2/useV2QuickPublish.ts:167-178`

这意味着这里存在一个很值得审计的功能差异：

> **界面快速发布可能没有覆盖旧 quick publish 的 system pre-publish 行为。**
