> **历史文档（2026-09-20 备注）**：本文描述的 vite.v1.app.config.ts / vite.v1.siyuan.config.ts 与 scripts/siyuan_build.py **已在旧界面退役时删除**。当前构建链：插件本体 vite.v2.config.ts → dist/；挂件/扩展/nginx/vercel → vite.webapp.config.ts（按 BUILD_TYPE）。下文保留当时事实，勿作为现行依据。
...
最重要的用户决策已经落实：迁移完成后，旧界面的 historical esbuild direct 构建链路直接移除，不保留 fallback。
...
注意：`pnpm-lock.yaml` 中仍会出现 Vite/Vitest/tsx/vercel 传递或 optional peer resolution 的 `esbuild`，这不是旧界面的 direct build chain，不能强删。
...
- `vite.v1.app.config.ts`
  - 旧界面 iframe/app、widget、nginx、extension、vercel app 构建。
...
- `vite.v1.siyuan.config.ts`
  - 旧界面 legacy SiYuan plugin CJS entry 构建。
...
- `scripts/dev.py` → 旧界面 prebuild 用 `vite.v1.siyuan.config.ts`，旧界面 watch 用 `vite.v1.app.config.ts`
...
- 不要恢复 `zhi-build`。
- 不要用 `pnpm dev -p siyuan` 验证插件本体。
- 不要用 mock/skip 来宣称平台验证通过。
- 不要把旧界面/package/widget/nginx/ext 全量 release packaging 作为本变更 merge blocker；它们是 release packaging 前 mandatory gate。
