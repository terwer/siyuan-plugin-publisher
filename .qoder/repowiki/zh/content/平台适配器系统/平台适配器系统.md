...
**更新** 新增 Docsify GitHub 和 GitLab Docsify 适配器，扩展静态站点平台支持范围
...
- Docsify 适配器：继承 CommonGithubApiAdaptor，重写 getYamlAdaptor 返回 DocsifyYamlConverterAdaptor，在 preEditPost 中处理 Front Matter 与 Markdown 的拼接，支持 Docsify 特有的 frontmatter 格式。
- GitLab Docsify 适配器：继承 CommonGitlabApiAdaptor，重用 DocsifyYamlConverterAdaptor，提供 GitLab 平台特定的配置与 API 调用支持。
- Hexo/Hugo/Jekyll/Vuepress/Vitepress/Quartz/Astro：通过 useXxxApi 返回适配器实例，适配器继承 BaseBlogApi，支持 YAML 转换器注入。
...
**更新** 新增 Docsify GitHub 和 GitLab Docsify 适配器显著扩展了静态站点平台的支持范围，为用户提供了更多文档托管选项。Docsify 适配器继承了 CommonGithubApiAdaptor 和 CommonGitlabApiAdaptor 的特性，同时针对 Docsify 的 frontmatter 格式进行了专门优化，确保与 Docsify 生态系统的完美集成。
