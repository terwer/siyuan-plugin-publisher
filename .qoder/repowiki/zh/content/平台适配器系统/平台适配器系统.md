...
- [src/adaptors/api/base/gitlab/commonGitlabApiAdaptor.ts](file://src/adaptors/api/base/gitlab/commonGitlabApiAdaptor.ts)
- [src/adaptors/api/base/github/commonGithubConfig.ts](file://src/adaptors/api/base/github/commonGithubConfig.ts)
</cite>

## 更新摘要
**所做更改**
- 新增 Docsify GitHub 适配器章节，详细介绍 Docsify 平台的适配器实现
- 新增 GitLab Docsify 适配器章节，说明 GitLab 平台上的 Docsify 支持
- 更新静态站点适配器分类，将 Docsify 纳入支持范围
- 扩展平台支持矩阵，包含新的 Docsify 生态系统适配器
- 新增 Docsify YAML 转换器的详细说明
- 新增 GitHub 和 GitLab 基础适配器的详细说明
...
## 简介
本系统采用适配器模式，通过统一的接口抽象（BlogAdaptor、WebAdaptor、YamlConvertAdaptor）屏蔽不同平台的差异，实现对多种平台的一致化发布能力。系统按平台类型与子类型进行分层组织，覆盖博客平台（Metaweblog、WordPress、Typecho 等）、静态站点（GitHub Pages、Hexo、Hugo、**Docsify** 等）、内容平台（语雀、Notion、Confluence 等）、Web 平台（知乎、CSDN、微信公众号等）以及文件系统适配器。

**更新** 新增 Docsify GitHub 和 GitLab Docsify 适配器，扩展静态站点平台支持范围。新增 GitHub 和 GitLab 基础适配器，为静态站点平台提供统一的文件操作能力。
...
### 静态站点适配器（GitHub Pages、Hexo、Hugo、**Docsify** 等）
**更新** Docsify 适配器现已加入静态站点平台支持

- **GitHub Docsify 适配器**：继承 CommonGithubApiAdaptor，重写 getYamlAdaptor 返回 DocsifyYamlConverterAdaptor，在 preEditPost 中处理 Front Matter 与 Markdown 的拼接，支持 Docsify 特有的 frontmatter 格式。该适配器基于 GitHub API 提供完整的文件操作能力，包括文件创建、编辑、删除和预览链接生成。
- **GitLab Docsify 适配器**：继承 CommonGitlabApiAdaptor，重用 DocsifyYamlConverterAdaptor，提供 GitLab 平台特定的配置与 API 调用支持。该适配器基于 GitLab API 提供文件操作功能，支持私有仓库和企业版 GitLab。
- **GitHub 基础适配器**：提供 GitHub 仓库的通用操作能力，包括文件树节点获取、图片上传、预览链接生成等。支持多种文件名规则和自定义配置。
- **GitLab 基础适配器**：提供 GitLab 项目的通用操作能力，包括仓库树节点获取、图片上传、预览链接生成等。支持代理客户端和自定义配置。
- **Docsify YAML 转换器**：专门处理 Docsify 平台的 YAML Front Matter 格式，支持 title、description、date 等标准字段，以及自定义动态配置。
- **GitLab Docsify YAML 转换器**：继承 DocsifyYamlConverterAdaptor，提供 GitLab 平台特定的 YAML 处理能力。

```mermaid
flowchart TD
Start(["进入 DocsifyApiAdaptor.preEditPost"]) --> Extract["提取 Docsify Front Matter 与 Markdown"]
Extract --> Process["处理 Docsify 特定格式"]
Process --> Reassemble["重新拼接为完整 Markdown"]
Reassemble --> SetDesc["根据页面类型设置 description"]
SetDesc --> End(["返回更新后的 Post"])
```

**图表来源**
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:28-59](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L28-L59)

**章节来源**
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:16-63](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L16-L63)
- [src/adaptors/api/docsify/docsifyConfig.ts:13-53](file://src/adaptors/api/docsify/docsifyConfig.ts#L13-L53)
- [src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts:15-97](file://src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts#L15-L97)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:16-63](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L16-L63)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts:14-56](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts#L14-L56)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts:15-20](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts#L15-L20)
...
- [src/adaptors/api/base/gitlab/commonGitlabApiAdaptor.ts:30-300](file://src/adaptors/api/base/gitlab/commonGitlabApiAdaptor.ts#L30-L300)
...
- 支持的静态站点类型：Hexo、Hugo、Jekyll、Vuepress、Vuepress2、Vitepress、Quartz、Astro、**Docsify**；默认类型使用 LocalSystemYamlConvertAdaptor。
...
- **Docsify 特定依赖**：Docsify 适配器依赖 GitHub/GitLab 基础适配器提供的文件操作能力，以及 DocsifyYamlConverterAdaptor 的 YAML 处理能力。
...
CommonGithubApiAdaptor --> CommonGithubConfig["CommonGithubConfig"]
CommonGitlabApiAdaptor --> GitlabdocsifyYamlConverterAdaptor
```
...
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts:19-19](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts#L19-L19)
...
- **Docsify 特定优化**：Docsify 适配器在处理 Front Matter 时采用高效的字符串拼接方式，避免不必要的 DOM 操作。
- **GitHub/GitLab 优化**：基础适配器提供缓存机制和批量操作支持，减少 API 调用次数。
...
- YAML 未生效：检查 YAML 策略类型与自定义 YAML 的属性前缀；确保在源码模式下正确同步。
- **Docsify 发布问题**：检查 Docsify Front Matter 格式是否符合规范；确认 Markdown 内容中包含正确的 frontmatter 标记；验证 Docsify YAML 转换器的配置项是否正确。
- **GitHub/GitLab 集成问题**：检查仓库权限和分支配置；确认 API 密钥有效；验证网络连接和代理设置。
...
- [src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts:28-70](file://src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts#L28-L70)
...
**更新** 新增 Docsify GitHub 和 GitLab Docsify 适配器显著扩展了静态站点平台的支持范围，为用户提供了更多文档托管选项。GitHub 和 GitLab 基础适配器为静态站点平台提供了统一的文件操作能力，支持多种静态站点生成器。Docsify 适配器继承了 CommonGithubApiAdaptor 和 CommonGitlabApiAdaptor 的特性，同时针对 Docsify 的 frontmatter 格式进行了专门优化，确保与 Docsify 生态系统的完美集成。
...
- 接口实现
  ...
  - **Docsify 特定实现**：继承 CommonGithubApiAdaptor 或 CommonGitlabApiAdaptor，重写 getYamlAdaptor 返回 DocsifyYamlConverterAdaptor，在 preEditPost 中处理 Docsify 特有的 frontmatter 格式。
  - **GitHub/GitLab 基础实现**：继承 BaseBlogApi，实现文件操作方法（newPost、editPost、deletePost、getPost），提供认证检查和图片上传功能。
- 配置文件编写
  - Web 平台配置：参考 ZhihuConfig，设置域名、中间件、页面类型、密码类型、知识空间等。
  - 文件系统配置：参考 LocalSystemConfig，设置存储路径、媒体路径、YAML 类型、图床服务等。
  - **Docsify 配置**：参考 DocsifyConfig，设置 Docsify 特定的配置项如默认路径、文件命名规则、图片存储路径等。
  - **GitHub/GitLab 配置**：参考 CommonGithubConfig，设置仓库信息、分支配置、预览链接等。
- 测试方法
  ...
  - **Docsify 测试**：验证 Front Matter 格式的正确性、Markdown 内容的拼接效果、YAML 转换器的兼容性。
  - **GitHub/GitLab 测试**：验证文件操作的正确性、认证机制的有效性和 API 调用的稳定性。
