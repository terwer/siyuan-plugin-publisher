...
- [xiaohongshuPlaceholder.ts](file://src/adaptors/web/xiaohongshu/xiaohongshuPlaceholder.ts)
...
- 平台适配器：按平台划分目录，如知乎(zhihu)、微信公众号(wechat)、掘金(juejin)、CSDN(csdn)、B站(bilibili)、小红书(xiaohongshu)
...
Xiaohongshu["XiaohongshuWebAdaptor<br/>小红书适配器"]
end
subgraph "配置层"
...
XiaohongshuCfg["XiaohongshuConfig"]
end
Base --> Zhihu
Base --> Wechat
Base --> Juejin
...
- [XiaohongshuWebAdaptor.ts:71-108](file://src/adaptors/web/xiaohongshu/XiaohongshuWebAdaptor.ts#L71-L108)
...
**更新** 新增小红书平台适配器章节，包含签名机制研究和实现细节

小红书适配器是本项目中最新加入的Web平台适配器，专门用于适配小红书创作者平台。由于小红书的签名机制极其复杂，该适配器目前处于实验性阶段，标记为`@deprecated`状态。

### 核心功能
- 登录校验：通过调用`/api/galaxy/user/info`接口验证用户登录状态
- 签名生成：提供基础的签名头生成框架，但核心签名算法仍在研究中
- 请求封装：封装了小红书API的请求方法，支持标准的HTTP方法

### 签名机制研究
根据研究报告显示，小红书的签名机制包含以下关键要素：

#### 请求头分析
- `x-s`：请求签名，前缀`XYS_` + Base64编码数据
- `x-s-common`：通用签名参数，约500+字符
- `x-t`：毫秒级时间戳
- `x-b3-traceid`：追踪ID，随机生成
...
#### 技术难点
- **鸡生蛋问题**：签名函数需要通过`/api/sec/v1/scripting` API获取，但请求这个API本身需要签名
- **动态脚本加载**：签名脚本不是静态存在于页面中，而是在特定条件下动态加载
- **反爬虫机制**：涉及设备指纹检测、行为分析、请求频率限制等多个层面
...
-private xiaohongshuFetch(url, headers, params, method, contentType) Promise~any~
-private xiaohongshuFormFetch(url, formData) Promise~any~
}
class XiaohongshuUtils {
+generateSignature(url, timestamp) Record~string,string~
...
XiaohongshuWebAdaptor --> XiaohongshuConfig : uses
```
...
### 配置特点
- API地址：`https://creator.xiaohongshu.com`
- 页面类型：Markdown
- 密码类型：Cookie
- 特性开关：标签和分类均不支持，知识空间不支持

### 使用方式
小红书适配器通过`useXiaohongshuWeb`自定义Hook提供，支持从配置或环境变量获取配置信息。
...
- [xiaohongshuPlaceholder.ts:12](file://src/adaptors/web/xiaohongshu/xiaohongshuPlaceholder.ts#L12)
- [xiaohongshu-signature-research.md:1-149](file://TODO/xiaohongshu-signature-research.md#L1-L149)
...
Base --> Xiaohongshu["XiaohongshuWebAdaptor"]
...
Common --> XiaohongshuCfg["XiaohongshuConfig"]
```
...
- 小红书签名问题
  - 症状：请求被拒绝，提示需要签名
  - 处理：当前签名机制尚未完全实现，建议使用手动登录获取Cookie的方式；关注后续版本更新
...
小红书平台适配器虽然目前仍处于实验阶段，但其复杂的签名机制研究为未来的功能完善奠定了坚实基础。在实际使用中，建议重点关注Cookie管理、跨域代理与图片直传策略，并结合平台特性进行参数与格式的适配，以获得最佳的发布体验。

对于小红书等具有复杂签名机制的平台，建议采用渐进式开发策略：先实现基础功能，再逐步完善签名机制，确保系统的稳定性和可靠性。