# 静态站点适配器

<cite>
**本文档引用的文件**
- [src/adaptors/index.ts](file://src/adaptors/index.ts)
- [src/adaptors/base/baseExtendApi.ts](file://src/adaptors/base/baseExtendApi.ts)
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts)
- [src/adaptors/web/base/baseWebApi.ts](file://src/adaptors/web/base/baseWebApi.ts)
- [src/adaptors/api/astro/astroApiAdaptor.ts](file://src/adaptors/api/astro/astroApiAdaptor.ts)
- [src/adaptors/api/hexo/hexoApiAdaptor.ts](file://src/adaptors/api/hexo/hexoApiAdaptor.ts)
- [src/adaptors/api/hugo/hugoApiAdaptor.ts](file://src/adaptors/api/hugo/hugoApiAdaptor.ts)
- [src/adaptors/api/jekyll/jekyllApiAdaptor.ts](file://src/adaptors/api/jekyll/jekyllApiAdaptor.ts)
- [src/adaptors/api/vitepress/vitepressApiAdaptor.ts](file://src/adaptors/api/vitepress/vitepressApiAdaptor.ts)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts)
- [src/adaptors/api/base/github/commonGithubConfig.ts](file://src/adaptors/api/base/github/commonGithubConfig.ts)
- [src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts](file://src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts)
- [src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts](file://src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts)
- [src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts](file://src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts)
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts)
- [src/adaptors/api/docsify/docsifyConfig.ts](file://src/adaptors/api/docsify/docsifyConfig.ts)
- [src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts](file://src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts)
- [src/adaptors/api/docsify/docsifyPlaceholder.ts](file://src/adaptors/api/docsify/docsifyPlaceholder.ts)
- [src/adaptors/api/docsify/useDocsifyApi.ts](file://src/adaptors/api/docsify/useDocsifyApi.ts)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyPlaceholder.ts](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyPlaceholder.ts)
- [src/adaptors/api/gitlab-docsify/useGitlabdocsifyApi.ts](file://src/adaptors/api/gitlab-docsify/useGitlabdocsifyApi.ts)
- [src/platforms/dynamicConfig.ts](file://src/platforms/dynamicConfig.ts)
</cite>

## 更新摘要
**所做更改**
- 新增 Docsify 平台支持章节，涵盖 GitHub 和 GitLab 变体
- 添加 Docsify API 适配器、配置系统、YAML 转换器的完整实现说明
- 更新架构图和组件关系，反映 Docsify 平台的集成
- 补充 Docsify 特定的功能特性，如知识空间支持、标签禁用等

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向"静态站点适配器"的技术文档，系统性阐述该适配器如何对接多种静态站点生成器（如 Hexo、Hugo、Jekyll、VitePress、Astro、Quartz、Docsify 等），并支持 GitHub Pages、GitLab Pages 等托管平台的集成。文档覆盖适配原理、YAML 配置转换、文件上传机制、Git 工作流集成（分支管理、提交策略、自动化部署）、以及不同静态站点生成器的差异化处理与通用解决方案。

**更新** 新增 Docsify 平台支持，包括 GitHub Docsify 和 GitLab Docsify 变体的完整集成方案。

## 项目结构
适配器采用"平台类型 + 子平台类型"的动态配置模型，通过统一入口按平台 key 分发到具体适配器，实现对多静态站点生成器与托管平台的统一接入。

```mermaid
graph TB
A["Adaptors 统一入口<br/>src/adaptors/index.ts"] --> B["动态配置<br/>src/platforms/dynamicConfig.ts"]
B --> C["GitHub 适配器基类<br/>src/adaptors/api/base/github/commonGithubApiAdaptor.ts"]
B --> D["GitLab 适配器基类<br/>src/adaptors/api/base/gitlab/commonGitlabApiAdaptor.ts"]
B --> E["网页适配器基类<br/>src/adaptors/web/base/baseWebApi.ts"]
B --> F["本地文件系统适配器<br/>src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts"]
C --> C1["Hexo 适配器<br/>src/adaptors/api/hexo/hexoApiAdaptor.ts"]
C --> C2["Hugo 适配器<br/>src/adaptors/api/hugo/hugoApiAdaptor.ts"]
C --> C3["Jekyll 适配器<br/>src/adaptors/api/jekyll/jekyllApiAdaptor.ts"]
C --> C4["VitePress 适配器<br/>src/adaptors/api/vitepress/vitepressApiAdaptor.ts"]
C --> C5["Astro 适配器<br/>src/adaptors/api/astro/astroApiAdaptor.ts"]
C --> C6["Docsify 适配器<br/>src/adaptors/api/docsify/docsifyApiAdaptor.ts"]
D --> D1["GitLab Hugo 适配器<br/>src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts"]
D --> D2["GitLab Jekyll 适配器<br/>src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts"]
D --> D3["GitLab Astro 适配器<br/>src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts"]
D --> D4["GitLab Docsify 适配器<br/>src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts"]
E --> E1["自定义站点适配器网页授权"]
F --> F1["本地文件系统适配器"]
```

**图表来源**
- [src/adaptors/index.ts:56-263](file://src/adaptors/index.ts#L56-L263)
- [src/platforms/dynamicConfig.ts:174-238](file://src/platforms/dynamicConfig.ts#L174-L238)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:28-47](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L28-L47)
- [src/adaptors/api/hexo/hexoApiAdaptor.ts:23-26](file://src/adaptors/api/hexo/hexoApiAdaptor.ts#L23-L26)
- [src/adaptors/api/hugo/hugoApiAdaptor.ts:23-26](file://src/adaptors/api/hugo/hugoApiAdaptor.ts#L23-L26)
- [src/adaptors/api/jekyll/jekyllApiAdaptor.ts:23-26](file://src/adaptors/api/jekyll/jekyllApiAdaptor.ts#L23-L26)
- [src/adaptors/api/vitepress/vitepressApiAdaptor.ts:23-26](file://src/adaptors/api/vitepress/vitepressApiAdaptor.ts#L23-L26)
- [src/adaptors/api/astro/astroApiAdaptor.ts:23-26](file://src/adaptors/api/astro/astroApiAdaptor.ts#L23-L26)
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:23-26](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L23-L26)
- [src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts:23-26](file://src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts#L23-L26)
- [src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts:23-26](file://src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts#L23-L26)
- [src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts:23-26](file://src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts#L23-L26)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:23-26](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L23-L26)
- [src/adaptors/web/base/baseWebApi.ts:36-63](file://src/adaptors/web/base/baseWebApi.ts#L36-L63)
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts:42-46](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts#L42-L46)

**章节来源**
- [src/adaptors/index.ts:56-263](file://src/adaptors/index.ts#L56-L263)
- [src/platforms/dynamicConfig.ts:174-238](file://src/platforms/dynamicConfig.ts#L174-L238)

## 核心组件
- 统一入口适配器：根据平台 key 解析子平台类型，分发到具体适配器或 YAML 转换器。
- 扩展基类：提供统一的预处理流程（文件名、摘要、分类、图片、YAML、正文、其他）与图片上传、外链替换、消息推送等通用能力。
- GitHub/GitLab 适配器基类：封装 GitHub/GitLab Pages 的通用操作（认证、树节点、发布、更新、删除、预览、媒体上传）。
- 静态站点生成器适配器：针对不同生成器（Hexo、Hugo、Jekyll、VitePress、Astro、Quartz、Docsify）定制 YAML 前言与正文处理。
- 本地文件系统适配器：将文章与媒体写入本地文件系统，支持多种 YAML 适配器与自动分类。
- 网页授权适配器基类：封装跨域代理、表单上传、Cookie 组合等网页授权场景。

**更新** 新增 Docsify 适配器组件，包括 Docsify API 适配器、Docsify 配置系统、Docsify YAML 转换器等全套组件。

**章节来源**
- [src/adaptors/index.ts:56-573](file://src/adaptors/index.ts#L56-L573)
- [src/adaptors/base/baseExtendApi.ts:55-739](file://src/adaptors/base/baseExtendApi.ts#L55-L739)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:28-352](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L28-L352)
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts:42-273](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts#L42-L273)
- [src/adaptors/web/base/baseWebApi.ts:36-256](file://src/adaptors/web/base/baseWebApi.ts#L36-L256)

## 架构总览
适配器整体采用"平台类型 + 子平台类型"动态路由，结合 YAML 转换器与静态站点生成器适配器，实现对 GitHub Pages、GitLab Pages 与本地文件系统的统一发布。

```mermaid
classDiagram
class Adaptors {
+getCfg(key,newCfg) BlogConfig
+getAdaptor(key,newCfg) BlogAdaptor|WebAdaptor
+getYamlAdaptor(key,newCfg) YamlConvertAdaptor
}
class BaseExtendApi {
+preEditPost(post,id,publishCfg) Post
+getCategories(keyword) CategoryInfo[]
+getTags() TagInfo[]
+newMediaObject(mediaObject) Attachment
+getImagesFromMd(id,markdown) ImageItem[]
+readFileToBase64(url) Base64Info
}
class CommonGithubApiAdaptor {
+checkAuth() boolean
+getUsersBlogs() UserBlog[]
+newPost(post,publish) string
+editPost(postid,post,publish) boolean
+deletePost(postid) boolean
+newMediaObject(mediaObject) Attachment
+getPreviewUrl(postid) string
}
class LocalSystemApiAdaptor {
+getYamlAdaptor() YamlConvertAdaptor
+preEditPost(post,id,publishCfg) Post
+newPost(post,publish) string
+newMediaObject(mediaObject) Attachment
}
class BaseWebApi {
+webFetch(url,headers,params,method,...) any
+webFormFetch(url,headers,formData,forceProxy) any
+buildCookie(cookies) string
}
class HexoApiAdaptor
class HugoApiAdaptor
class JekyllApiAdaptor
class VitepressApiAdaptor
class AstroApiAdaptor
class DocsifyApiAdaptor
class GitlabhugoApiAdaptor
class GitlabjekyllApiAdaptor
class GitlabastroApiAdaptor
class GitlabdocsifyApiAdaptor
Adaptors --> BaseExtendApi : "委托"
BaseExtendApi --> CommonGithubApiAdaptor : "在GitHub场景下使用"
BaseExtendApi --> LocalSystemApiAdaptor : "在本地FS场景下使用"
BaseExtendApi --> BaseWebApi : "在网页授权场景下使用"
CommonGithubApiAdaptor <|-- HexoApiAdaptor
CommonGithubApiAdaptor <|-- HugoApiAdaptor
CommonGithubApiAdaptor <|-- JekyllApiAdaptor
CommonGithubApiAdaptor <|-- VitepressApiAdaptor
CommonGithubApiAdaptor <|-- AstroApiAdaptor
CommonGithubApiAdaptor <|-- DocsifyApiAdaptor
CommonGithubApiAdaptor <|-- GitlabhugoApiAdaptor
CommonGithubApiAdaptor <|-- GitlabjekyllApiAdaptor
CommonGithubApiAdaptor <|-- GitlabastroApiAdaptor
CommonGithubApiAdaptor <|-- GitlabdocsifyApiAdaptor
```

**图表来源**
- [src/adaptors/index.ts:56-573](file://src/adaptors/index.ts#L56-L573)
- [src/adaptors/base/baseExtendApi.ts:55-739](file://src/adaptors/base/baseExtendApi.ts#L55-L739)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:28-352](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L28-L352)
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts:42-273](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts#L42-L273)
- [src/adaptors/web/base/baseWebApi.ts:36-256](file://src/adaptors/web/base/baseWebApi.ts#L36-L256)
- [src/adaptors/api/hexo/hexoApiAdaptor.ts:23-63](file://src/adaptors/api/hexo/hexoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/hugo/hugoApiAdaptor.ts:23-63](file://src/adaptors/api/hugo/hugoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/jekyll/jekyllApiAdaptor.ts:23-63](file://src/adaptors/api/jekyll/jekyllApiAdaptor.ts#L23-L63)
- [src/adaptors/api/vitepress/vitepressApiAdaptor.ts:23-63](file://src/adaptors/api/vitepress/vitepressApiAdaptor.ts#L23-L63)
- [src/adaptors/api/astro/astroApiAdaptor.ts:23-62](file://src/adaptors/api/astro/astroApiAdaptor.ts#L23-L62)
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:23-63](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts:23-62](file://src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts#L23-L62)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L23-L63)

## 详细组件分析

### 统一入口与动态路由
- 根据平台 key 解析子平台类型，分发到对应适配器或 YAML 转换器。
- 支持 GitHub/Hugo、GitHub/Jekyll、GitHub/VitePress、GitHub/Astro、GitHub/Docsify、GitLab/Hugo、GitLab/Jekyll、GitLab/Astro、GitLab/Docsify 等多子平台。
- 未匹配时返回空配置，避免运行时异常。

**更新** 新增 GitHub/Docsify 和 GitLab/Docsify 子平台支持。

**章节来源**
- [src/adaptors/index.ts:65-263](file://src/adaptors/index.ts#L65-L263)
- [src/platforms/dynamicConfig.ts:397-418](file://src/platforms/dynamicConfig.ts#L397-L418)

### 扩展基类：统一预处理与图片处理
- 预处理流程：文件名规则、摘要同步、路径分类合并、图片上传、Markdown 正文处理、YAML 生成与注入、其他属性补全。
- 图片处理：支持外部图床（PicGO）与平台自带上传；支持 Confluence 宏替换与链接替换；自动识别本地/远程图片并进行 Base64 转换。
- 外链替换：将思源笔记外链替换为目标平台预览链接，支持忽略未发布的外链。
- YAML 处理：根据策略（自动生成/手动维护）选择对应 YAML 转换器，确保 Front Matter 与正文正确拼接。

```mermaid
flowchart TD
Start(["进入预处理"]) --> Filename["处理文件名规则"]
Filename --> Desc["同步摘要"]
Desc --> Categories["合并路径分类"]
Categories --> Pictures["图片处理与上传"]
Pictures --> Md["Markdown 正文处理"]
Md --> Yaml["YAML 生成/合并"]
Yaml --> Other["其他属性补全"]
Other --> End(["返回 Post"])
```

**图表来源**
- [src/adaptors/base/baseExtendApi.ts:90-456](file://src/adaptors/base/baseExtendApi.ts#L90-L456)

**章节来源**
- [src/adaptors/base/baseExtendApi.ts:90-739](file://src/adaptors/base/baseExtendApi.ts#L90-L739)

### GitHub Pages 适配器基类
- 认证检查：通过临时文件发布与删除验证权限。
- 树节点与博客列表：基于仓库树结构获取默认路径与仓库链接。
- 发布/更新/删除：支持路径变更时的移动与重试发布；媒体上传支持 base64。
- 预览 URL：支持用户、仓库、分支、文档路径占位符替换。
- 图片路径策略：支持文档路径占位、相对路径与默认路径三种模式。

```mermaid
sequenceDiagram
participant U as "用户"
participant A as "CommonGithubApiAdaptor"
participant G as "GitHub API"
U->>A : "发布文章"
A->>A : "计算保存路径与文件名"
A->>G : "publishGithubPage(路径, 内容)"
G-->>A : "返回内容路径"
A-->>U : "发布成功"
Note over A,G : "失败时尝试删除旧文件后重发"
```

**图表来源**
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:86-128](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L86-L128)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:251-309](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L251-L309)

**章节来源**
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:28-352](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L28-L352)
- [src/adaptors/api/base/github/commonGithubConfig.ts:17-112](file://src/adaptors/api/base/github/commonGithubConfig.ts#L17-L112)

### GitLab Pages 适配器基类
- 与 GitHub 适配器基类类似，提供 GitLab Pages 的通用发布、更新、删除、媒体上传与预览 URL 生成能力。
- 静态站点生成器适配器（GitLab Hugo/Jekyll/Astro/Docsify）复用该基类，实现差异化 YAML 处理。

**章节来源**
- [src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts:23-62](file://src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts#L23-L62)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L23-L63)

### 静态站点生成器适配器
- Hexo/Hugo/Jekyll/VitePress/Astro/Docsify 适配器均继承 GitHub 适配器基类，重写 YAML 转换器并定制正文处理（提取 Front Matter，保留正文）。
- Quartz 适配器同样遵循相同模式，确保与平台 YAML 规范一致。

**更新** 新增 Docsify 适配器，支持 Docsify 特定的 YAML 格式和正文处理逻辑。

**章节来源**
- [src/adaptors/api/hexo/hexoApiAdaptor.ts:23-63](file://src/adaptors/api/hexo/hexoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/hugo/hugoApiAdaptor.ts:23-63](file://src/adaptors/api/hugo/hugoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/jekyll/jekyllApiAdaptor.ts:23-63](file://src/adaptors/api/jekyll/jekyllApiAdaptor.ts#L23-L63)
- [src/adaptors/api/vitepress/vitepressApiAdaptor.ts:23-63](file://src/adaptors/api/vitepress/vitepressApiAdaptor.ts#L23-L63)
- [src/adaptors/api/astro/astroApiAdaptor.ts:23-62](file://src/adaptors/api/astro/astroApiAdaptor.ts#L23-L62)
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:23-63](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L23-L63)

### Docsify 平台适配器

#### Docsify API 适配器
Docsify 适配器专门处理 Docsify 静态站点生成器的特殊需求，支持 Markdown 格式的发布和 YAML 前言处理。

- YAML 转换：使用 DocsifyYamlConverterAdaptor 处理 YAML 前言，支持 title、description、date 字段
- 正文处理：提取并保留 Markdown 正文，支持 Front Matter 注入
- 发布格式：根据页面类型选择 Markdown 或 HTML 格式发布
- 知识空间：支持树形单选知识空间类型，提供发布目录管理

#### Docsify 配置系统
Docsify 配置继承自 GitHub 配置，针对 Docsify 特性进行优化：

- 默认路径：docs 目录作为默认文档存储路径
- 文件命名：使用 [slug].md 格式
- 图片存储：docs/images 目录，链接路径为 /images
- 页面类型：默认 Markdown 页面类型
- 功能限制：禁用标签和分类功能，支持知识空间
- 预览 URL：支持 Docsify 特定的预览链接格式

#### Docsify YAML 转换器
专门的 YAML 转换器处理 Docsify 特定的 YAML 格式：

- 必需字段：title（标题）、description（描述）、date（日期）
- 日期格式：使用中文日期格式
- 动态配置：支持通过 dynYamlCfg 参数添加自定义 YAML 字段
- 双向转换：支持 YAML 到 Post 属性的转换和 Post 到 YAML 的生成

#### Docsify 占位符系统
Docsify 占位符继承自 GitHub 占位符，提供 Docsify 特定的提示信息：

- 知识空间只读提示：说明 Docsify 平台暂不支持修改发布目录
- 用户体验：提供清晰的操作指导和限制说明

**章节来源**
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:23-63](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L23-L63)
- [src/adaptors/api/docsify/docsifyConfig.ts:19-52](file://src/adaptors/api/docsify/docsifyConfig.ts#L19-L52)
- [src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts:25-97](file://src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts#L25-L97)
- [src/adaptors/api/docsify/docsifyPlaceholder.ts:18-21](file://src/adaptors/api/docsify/docsifyPlaceholder.ts#L18-L21)
- [src/adaptors/api/docsify/useDocsifyApi.ts:22-94](file://src/adaptors/api/docsify/useDocsifyApi.ts#L22-L94)

### GitLab Docsify 适配器

#### GitLab Docsify API 适配器
GitLab Docsify 适配器继承自 GitLab 基类，提供 GitLab Pages 平台上的 Docsify 支持：

- 配置继承：继承 DocsifyConfig 基础配置
- GitLab 特性：支持 GitLab 主机和 API URL 配置
- 预览链接：使用 GitLab 特定的预览链接格式
- 认证配置：支持 GitLab 令牌设置 URL

#### GitLab Docsify 配置系统
GitLab Docsify 配置针对 GitLab Pages 进行优化：

- 主机配置：支持自定义 GitLab 主机地址
- API 配置：支持自定义 GitLab API URL
- 令牌设置：使用 GitLab 特定的令牌设置路径
- 预览 URL：支持 GitLab Pages 的 blob 预览格式

#### GitLab Docsify YAML 转换器
与 Docsify YAML 转换器功能相同，但继承自 GitLab 基类，确保与 GitLab Pages 的兼容性。

#### GitLab Docsify 占位符系统
GitLab Docsify 占位符提供 GitLab 特定的提示信息：

- GitLab 环境提示：说明平台为 GitLab Docsify 变体
- 操作指导：提供 GitLab Pages 特定的操作指导

**章节来源**
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts:20-56](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts#L20-L56)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts:19-21](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyYamlConverterAdaptor.ts#L19-L21)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyPlaceholder.ts:18-21](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyPlaceholder.ts#L18-L21)
- [src/adaptors/api/gitlab-docsify/useGitlabdocsifyApi.ts:22-94](file://src/adaptors/api/gitlab-docsify/useGitlabdocsifyApi.ts#L22-L94)

### 本地文件系统适配器
- 初始化存储路径与媒体路径，确保目录存在。
- 根据 fsYamlType 动态选择 YAML 转换器（Hexo/Hugo/Jekyll/Vuepress/Vuepress2/Vitepress/Quartz/Astro/Docsify/Default）。
- 预处理阶段支持自动分类与路径替换，发布阶段直接写入文件与媒体文件。

**更新** Docsify 适配器已集成到本地文件系统适配器的 YAML 转换器选择中。

**章节来源**
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts:42-273](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts#L42-L273)

### 网页授权适配器基类
- 提供 webFetch 与 webFormFetch，支持代理与 CORS 两种模式自动切换。
- 支持 Cookie 组合与表单上传，适配多种自定义站点授权场景。

**章节来源**
- [src/adaptors/web/base/baseWebApi.ts:36-256](file://src/adaptors/web/base/baseWebApi.ts#L36-L256)

## 依赖关系分析
- 平台类型与子平台类型：通过动态配置枚举与 key 解析，实现平台级与子平台级的解耦。
- 适配器分层：统一入口 -> 基类 -> 具体适配器 -> YAML 转换器，职责清晰、扩展性强。
- 外部依赖：GitHub/GitLab 中间件、跨域代理、图床桥接、Markdown/HTML 转换工具等。

```mermaid
graph LR
DC["动态配置<br/>dynamicConfig.ts"] --> IDX["适配器入口<br/>index.ts"]
IDX --> EXT["扩展基类<br/>baseExtendApi.ts"]
EXT --> CGH["GitHub 基类<br/>commonGithubApiAdaptor.ts"]
EXT --> LFS["本地FS适配器<br/>LocalSystemApiAdaptor.ts"]
EXT --> WEB["网页基类<br/>baseWebApi.ts"]
CGH --> HEX["Hexo 适配器"]
CGH --> HUG["Hugo 适配器"]
CGH --> JEK["Jekyll 适配器"]
CGH --> VIT["VitePress 适配器"]
CGH --> AST["Astro 适配器"]
CGH --> DOC["Docsify 适配器"]
CGH --> GLH["GitLab Hugo 适配器"]
CGH --> GLJ["GitLab Jekyll 适配器"]
CGH --> GLA["GitLab Astro 适配器"]
CGH --> GLD["GitLab Docsify 适配器"]
```

**图表来源**
- [src/platforms/dynamicConfig.ts:174-238](file://src/platforms/dynamicConfig.ts#L174-L238)
- [src/adaptors/index.ts:56-573](file://src/adaptors/index.ts#L56-L573)
- [src/adaptors/base/baseExtendApi.ts:55-739](file://src/adaptors/base/baseExtendApi.ts#L55-L739)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:28-352](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L28-L352)
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts:42-273](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts#L42-L273)
- [src/adaptors/web/base/baseWebApi.ts:36-256](file://src/adaptors/web/base/baseWebApi.ts#L36-L256)

**章节来源**
- [src/platforms/dynamicConfig.ts:174-238](file://src/platforms/dynamicConfig.ts#L174-L238)
- [src/adaptors/index.ts:56-573](file://src/adaptors/index.ts#L56-L573)

## 性能考量
- 图片处理：优先使用平台自带上传以减少跨域与代理开销；批量上传时建议并发控制与去重。
- YAML 处理：尽量使用平台专用 YAML 转换器，避免重复解析与拼接造成的额外 CPU 开销。
- 文件系统写入：本地 FS 发布建议异步写入与错误重试，避免阻塞主线程。
- 预览 URL：缓存常用预览链接，减少重复拼接与替换成本。
- Docsify 特殊处理：由于 Docsify 不支持标签和分类，发布前会自动禁用相关功能，减少不必要的处理开销。

**更新** Docsify 平台的性能优化考虑，包括标签分类禁用和知识空间优化。

## 故障排查指南
- 认证失败：检查令牌权限与仓库分支配置；使用认证检查接口快速定位问题。
- 发布失败重试：当首次发布失败时会尝试删除旧文件后重发；若仍失败，请检查路径与权限。
- 图片上传异常：确认图片路径策略与跨域代理配置；Confluence 场景注意宏替换优先级。
- 外链未发布：若启用严格模式，未发布的外链会抛出错误；可在偏好设置中调整忽略策略。
- YAML 不生效：检查 YAML 策略与编辑模式；源码模式下需确保 YAML 正确注入正文。
- Docsify 发布问题：确认使用正确的 Docsify YAML 格式；检查知识空间配置；验证 Markdown 文件命名规则。

**更新** 新增 Docsify 平台相关的故障排查指导。

**章节来源**
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:49-64](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L49-L64)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:112-125](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L112-L125)
- [src/adaptors/base/baseExtendApi.ts:535-551](file://src/adaptors/base/baseExtendApi.ts#L535-L551)
- [src/adaptors/base/baseExtendApi.ts:686-689](file://src/adaptors/base/baseExtendApi.ts#L686-L689)

## 结论
该静态站点适配器通过统一入口与动态配置，实现了对多种静态站点生成器与托管平台的标准化接入。借助扩展基类提供的统一预处理与图片处理能力，以及 GitHub/GitLab 基类的通用发布流程，适配器在保证一致性的同时具备良好的可扩展性。配合 YAML 转换器与平台差异化的正文处理，能够高效完成从内容到静态站点的完整发布链路。

**更新** 新增 Docsify 平台支持，进一步丰富了静态站点生成器的适配能力，包括 GitHub Docsify 和 GitLab Docsify 变体，为用户提供更多部署选项和更好的用户体验。

## 附录

### GitHub Pages 集成要点
- 配置项：用户名、仓库、分支、默认路径、默认提交信息、作者/邮箱、预览 URL 占位符。
- 发布流程：计算路径与文件名 -> 发布 -> 失败重试 -> 返回内容路径。
- 图片上传：支持 base64 直传与链接生成，支持文档路径占位与相对路径策略。

**章节来源**
- [src/adaptors/api/base/github/commonGithubConfig.ts:17-112](file://src/adaptors/api/base/github/commonGithubConfig.ts#L17-L112)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:86-128](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L86-L128)
- [src/adaptors/api/base/github/commonGithubApiAdaptor.ts:251-309](file://src/adaptors/api/base/github/commonGithubApiAdaptor.ts#L251-L309)

### GitLab Pages 集成要点
- 与 GitHub Pages 类似的通用发布流程，但面向 GitLab Pages。
- 静态站点生成器适配器（Hugo/Jekyll/Astro/Docsify）复用该基类，实现差异化 YAML 处理。

**更新** GitLab Docsify 适配器现已支持 Docsify 平台的 GitLab Pages 部署。

**章节来源**
- [src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-hugo/gitlabhugoApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-jekyll/gitlabjekyllApiAdaptor.ts#L23-L63)
- [src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts:23-62](file://src/adaptors/api/gitlab-astro/gitlabastroApiAdaptor.ts#L23-L62)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:23-63](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L23-L63)

### YAML 配置转换
- 平台专用 YAML 转换器：Hexo/Hugo/Jekyll/VitePress/Astro/Quartz/Docsify/Default。
- 转换策略：自动生成（基于 Post 属性生成 YAML）与手动维护（保留用户 YAML 并与平台字段合并）。
- 正文处理：提取 Front Matter，保留正文，确保发布内容符合平台规范。

**更新** Docsify YAML 转换器现已集成到平台专用 YAML 转换器列表中。

**章节来源**
- [src/adaptors/base/baseExtendApi.ts:360-456](file://src/adaptors/base/baseExtendApi.ts#L360-L456)
- [src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts:70-104](file://src/adaptors/fs/LocalSystem/LocalSystemApiAdaptor.ts#L70-L104)
- [src/adaptors/api/hexo/hexoApiAdaptor.ts:24-26](file://src/adaptors/api/hexo/hexoApiAdaptor.ts#L24-L26)
- [src/adaptors/api/hugo/hugoApiAdaptor.ts:24-26](file://src/adaptors/api/hugo/hugoApiAdaptor.ts#L24-L26)
- [src/adaptors/api/jekyll/jekyllApiAdaptor.ts:24-26](file://src/adaptors/api/jekyll/jekyllApiAdaptor.ts#L24-L26)
- [src/adaptors/api/vitepress/vitepressApiAdaptor.ts:24-26](file://src/adaptors/api/vitepress/vitepressApiAdaptor.ts#L24-L26)
- [src/adaptors/api/astro/astroApiAdaptor.ts:24-26](file://src/adaptors/api/astro/astroApiAdaptor.ts#L24-L26)
- [src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts:28-97](file://src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts#L28-L97)

### Git 工作流与自动化部署（建议）
- 分支管理：使用独立分支（如 docs 或 gh-pages/gitlab-pages）存放静态站点内容，主分支保持源码。
- 提交策略：采用原子提交（单文件或小范围变更），提交信息包含简要描述与关联 issue。
- 自动化部署：在 CI 中执行构建与发布，触发条件可设为标签推送或主分支合并。
- 安全性：使用仓库环境变量或 CI 凭证，避免硬编码令牌；限制令牌权限最小化。

### Docsify 平台集成要点
- 配置项：GitHub 用户名、仓库、分支、中间件 URL、默认路径、文件命名规则。
- 发布流程：Docsify 特定的 YAML 处理 -> 正文提取与注入 -> Markdown/HTML 格式选择 -> 发布到指定路径。
- 功能特性：支持知识空间（树形单选）、禁用标签和分类、内置图床服务。
- 预览 URL：支持 Docsify 特定的预览链接格式，便于实时查看发布效果。

**更新** 新增 Docsify 平台的完整集成指导，包括配置、发布流程和功能特性说明。

**章节来源**
- [src/adaptors/api/docsify/docsifyConfig.ts:19-52](file://src/adaptors/api/docsify/docsifyConfig.ts#L19-L52)
- [src/adaptors/api/docsify/docsifyApiAdaptor.ts:28-60](file://src/adaptors/api/docsify/docsifyApiAdaptor.ts#L28-L60)
- [src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts:28-97](file://src/adaptors/api/docsify/docsifyYamlConverterAdaptor.ts#L28-L97)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts:20-56](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyConfig.ts#L20-L56)
- [src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts:28-60](file://src/adaptors/api/gitlab-docsify/gitlabdocsifyApiAdaptor.ts#L28-L60)