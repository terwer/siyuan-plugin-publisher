...
该基类的设计理念是"统一入口，平台特化"，通过继承机制让具体适配器专注于平台特定功能的实现，同时共享通用的预处理逻辑和基础设施。
...
PicgoBridge-->>BaseExtendApi : 返回图片URL
...
BaseExtendApi实现了完善的错误处理和日志记录机制：

| 错误类型 | 处理策略 | 日志级别 |
|---------|---------|---------|
| 图片上传失败 | 记录错误并通知用户 | ERROR |
| 外链引用未发布 | 抛出异常并提供解决方案 | ERROR |
| YAML适配器缺失 | 使用默认YAML生成 | WARN |
| 图床服务未配置 | 跳过图片处理并通知 | INFO |
