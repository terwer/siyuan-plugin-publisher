...
- **增强的错误处理机制**：支持图片上传错误的收集与展示，提升用户体验
...
E --> H["基础适配器<br/>src/adaptors/api/base/baseBlogApi.ts"]
H --> I["扩展功能API<br/>src/adaptors/base/baseExtendApi.ts"]
C --> J["配置存储Pinia<br/>src/stores/usePublishSettingStore.ts"]
C --> K["动态配置模型<br/>src/platforms/dynamicConfig.ts"]
C --> L["内容处理工具<br/>src/utils/luteUtil.ts / src/utils/mdUtils.ts / src/utils/ImageUtils.ts"]
C --> M["发布配置类型<br/>src/types/IPublishCfg.ts"]
```

图示来源
- [src/main.ts:1-22](file://src/main.ts#L1-L22)
- [src/publisherAppInstance.ts:1-50](file://src/publisherAppInstance.ts#L1-L50)
- [src/composables/usePublish.ts:1-566](file://src/composables/usePublish.ts#L1-L566)
...
- [src/adaptors/api/hexo/hexoApiAdaptor.ts:1-63](file://src/adaptors/api/hexo/hexoApiAdaptor.ts#L1-L63)
- [src/adaptors/api/base/baseBlogApi.ts:1-205](file://src/adaptors/api/base/baseBlogApi.ts#L1-L205)
- [src/adaptors/base/baseExtendApi.ts:520-586](file://src/adaptors/base/baseExtendApi.ts#L520-L586)
...
- **增强的错误处理机制**：支持图片上传错误的收集与展示，通过`imageUploadErrors`属性将错误信息传递给上层UI。
...
participant Base as "BaseBlogApi<br/>基础适配器"
participant Ext as "BaseExtendApi<br/>扩展功能"
...
Plat->>Ext : 图片上传处理
Ext-->>Plat : 返回处理后的post
Plat-->>Pub : 返回post(含imageUploadErrors)
Pub->>Pub : 检查imageUploadErrors属性
Pub->>Store : 写入/更新发布元信息
Pub-->>UI : 返回发布状态/预览链接/错误信息
```

图示来源
- [src/composables/usePublish.ts:105-124](file://src/composables/usePublish.ts#L105-L124)
...
- **增强的错误处理**：在预处理阶段检查`preEditPost`返回的post对象中的`imageUploadErrors`属性，将图片上传错误收集到`singleFormData.errMsg`中供UI显示。
...
PreEdit --> CheckErrors{"检查imageUploadErrors"}
CheckErrors --> |有错误| CollectErr["收集错误到errMsg"]
CheckErrors --> |无错误| IsAdd{"是否首次发布?"}
CollectErr --> IsAdd
...
图示来源
- [src/composables/usePublish.ts:105-124](file://src/composables/usePublish.ts#L105-L124)
- [src/composables/usePublish.ts:116-121](file://src/composables/usePublish.ts#L116-L121)
- [src/composables/usePublish.ts:127-177](file://src/composables/usePublish.ts#L127-L177)
- [src/composables/usePublish.ts:339-349](file://src/composables/usePublish.ts#L339-L349)
...
### 基础适配器与扩展功能（BaseBlogApi & BaseExtendApi）
- **BaseBlogApi**：提供统一的API访问接口，继承自 BlogApi，封装代理请求、认证检查等功能。
- **BaseExtendApi**：提供扩展功能，包括图片上传处理、链接替换、外链处理等，支持`imageUploadErrors`错误收集机制。
- **增强的图片处理**：在图片上传过程中捕获异常并收集到`post.imageUploadErrors`数组中，便于上层UI展示。
...
- HexoApiAdaptor：继承自 CommonGithubApiAdaptor，提供 Hexo 特定的预处理逻辑。
...
- 组件耦合与内聚
  ...
  - **增强的错误处理**：BaseExtendApi 与 usePublish 之间形成松耦合的错误传播机制。
- 外部依赖
...
Adp --> Base["baseBlogApi.ts"]
Base --> Ext["baseExtendApi.ts"]
Ext --> Plat1["useHexoApi.ts"]
Ext --> Plat2["useLocalSystemApi.ts"]
...
- **增强的错误处理性能**
  - 图片上传错误收集采用数组累积方式，避免异常抛出影响整体流程性能。
  - 错误信息在预处理阶段统一收集，减少后续处理的复杂度。
...
- **图片上传错误**
  - 现象：发布成功但出现图片上传警告，UI 显示图片上传错误详情
  - 处理：检查图床配置、网络连接、图片格式支持；查看 UI 中的详细错误信息
  - 影响：图片上传失败不影响文章发布，但会影响最终显示效果
...
## 结论
发布引擎通过"配置驱动 + 适配器模式"实现了对多平台的统一接入，具备完善的发布生命周期管理与动态配置能力。结合内容处理工具链，能够满足从 Markdown 到多平台格式的转换需求。**新增的增强错误处理机制**使得图片上传失败等非致命错误能够在不影响整体发布流程的情况下被用户感知，提升了用户体验。未来可在并发与重试方面进一步增强，以提升大规模发布的稳定性与效率。
...
- 单篇发布
  ...
  - **新增**：如果存在图片上传错误，UI 将显示详细的错误信息供用户处理。
- 删除文章
...
### 错误处理机制详解
**增强的图片上传错误处理**是本次更新的核心改进：

1. **错误收集机制**
   - 在`baseExtendApi.ts`中，图片上传异常被捕获并收集到`post.imageUploadErrors`数组中
   - 支持多张图片同时上传失败的情况，错误信息会被累积

2. **错误传播流程**
   - 适配器在`preEditPost`中调用`baseExtendApi.preEditPost`
   - `baseExtendApi`处理图片上传，遇到错误时收集到`post.imageUploadErrors`
   - 返回的post对象包含错误信息，供上层处理

3. **UI展示机制**
   - `usePublish.ts`检查`finalPost.imageUploadErrors`属性
   - 将错误信息合并到`singleFormData.errMsg`中
   - UI层根据状态显示图片上传警告或错误详情

4. **用户体验提升**
   - 图片上传失败不影响文章发布主流程
   - 用户可以清楚地看到具体哪些图片上传失败及失败原因
   - 支持手动重新上传或跳过有问题的图片

**章节来源**
- [src/composables/usePublish.ts:116-121](file://src/composables/usePublish.ts#L116-L121)
- [src/adaptors/base/baseExtendApi.ts:536-540](file://src/adaptors/base/baseExtendApi.ts#L536-L540)