...
- [src/adaptors/web/yuqueweb/YuquewebConfig.ts](file://src/adaptors/web/yuqueweb/YuquewebConfig.ts)
- [src/adaptors/web/yuqueweb/useYuquewebWeb.ts](file://src/adaptors/web/yuqueweb/useYuquewebWeb.ts)
- [src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts](file://src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts)
- [src/adaptors/web/yuqueweb/YuquewebPostMeta.ts](file://src/adaptors/web/yuqueweb/YuquewebPostMeta.ts)
- [src/components/set/publish/singleplatform/web/YuquewebSetting.vue](file://src/components/set/publish/singleplatform/web/YuquewebSetting.vue)
- [openspec/specs/yuque-web-publishing/spec.md](file://openspec/specs/yuque-web-publishing/spec.md)
...
## 更新摘要
**变更内容**
- 新增语雀网页版（Yuque Web）平台支持，扩展了平台能力范围
- 更新社交媒体平台分类，添加语雀网页版作为独立的网页授权平台
- 完善平台配置说明，包含 Cookie 授权方式和知识库支持功能
...
**更新** 本次更新新增了语雀网页版平台支持，扩展了平台能力范围，为用户提供更多发布选择。
...
B --> E["网页适配器配置<br/>src/adaptors/web/*"]
E --> F["语雀网页版适配器<br/>src/adaptors/web/yuqueweb/*"]
G["发布设置入口<br/>docs/发布设置.md"] --> H["插件主页/帮助文档<br/>README_zh_CN.md"]
```
...
- [src/adaptors/web/yuqueweb/YuquewebConfig.ts:16-38](file://src/adaptors/web/yuqueweb/YuquewebConfig.ts#L16-L38)
...
**更新** 新增语雀网页版平台（Custom_Yuqueweb），作为独立的网页授权平台，与现有的语雀 API 平台（Common_Yuque）并存。
...
- 子类型维度：SubPlatformType（如 Common_Yuque、Github_Hexo、Gitlab_Jekyll、Metaweblog_Cnblogs、Wordpress_Wordpress、Custom_Zhihu 等）
...
### 社交媒体（知乎、CSDN、微信公众号、简书、掘金、B站、Halo网页版、语雀网页版）
- 知乎（Zhihu）
  - 平台类型：Custom
...
- **语雀网页版（Yuqueweb）**
  - **平台类型：Custom**
  - **特点：网页授权（WEBSITE），使用 Cookie 认证，支持知识库选择、文档新建/更新/删除、图片上传、预览 URL**
  - **适用场景：语雀知识库内容发布，替代 API Token 方式**
  - **配置要点：登录URL为 https://www.yuque.com/login，域名 yuque.com，使用 Cookie 作为密码，支持知识库选择**
  - **参考配置：[YuquewebConfig.ts:16-38](file://src/adaptors/web/yuqueweb/YuquewebConfig.ts#L16-L38)**
  - **适配器：[YuquewebWebAdaptor.ts:64-200](file://src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts#L64-L200)**

**平台图标与入口**
- 图标来源：svgIcons（在预定义中以 platformIcon 字段引用）
- 入口：发布设置页面（/#/manage）

**更新** 新增语雀网页版平台，作为独立的网页授权平台，与现有的语雀 API 平台并存，提供基于 Cookie 的认证方式。

**章节来源**
- [src/platforms/pre.ts:337-438](file://src/platforms/pre.ts#L337-L438)
- [src/platforms/pre.ts:533-545](file://src/platforms/pre.ts#L533-L545)
- [src/platforms/PreConstants.ts:10-19](file://src/platforms/PreConstants.ts#L10-L19)
- [src/adaptors/web/yuqueweb/YuquewebConfig.ts:16-38](file://src/adaptors/web/yuqueweb/YuquewebConfig.ts#L16-L38)
- [src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts:64-200](file://src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts#L64-L200)
- [docs/发布设置.md:5](file://docs/发布设置.md#L5)
...
- **新增** 语雀网页版适配器（YuquewebWebAdaptor）提供独立的 Cookie 授权和文档操作能力
...
AC --> YW["语雀网页版适配器<br/>YuquewebWebAdaptor"]
YW --> YWC["语雀网页版配置<br/>YuquewebConfig"]
YW --> YWP["语雀网页版设置<br/>YuquewebSetting.vue"]
```
...
- [src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts:64-200](file://src/adaptors/web/yuqueweb/YuquewebWebAdaptor.ts#L64-L200)
- [src/adaptors/web/yuqueweb/YuquewebConfig.ts:16-38](file://src/adaptors/web/yuqueweb/YuquewebConfig.ts#L16-L38)
- [src/components/set/publish/singleplatform/web/YuquewebSetting.vue:10-70](file://src/components/set/publish/singleplatform/web/YuquewebSetting.vue#L10-L70)
...
- **新增** 语雀网页版平台使用 Cookie 授权，相比 API Token 方式可能具有更好的稳定性，特别是在 API 限流情况下。
...
- 常见问题定位
  ...
  - **新增** 语雀网页版平台：若 Cookie 失效，需要重新登录语雀并重新读取 Cookie；确保知识库有写入权限
...
## 结论
本插件通过统一的动态配置模型与平台预定义，实现了对20+平台的标准化接入。用户可根据自身需求选择合适平台：博客平台适合个人/企业博客；内容平台适合知识库与协作；静态站点适合文档/个人站点；社交媒体适合内容分发；文件系统适合本地部署。

**更新** 新增的语雀网页版平台为用户提供了基于 Cookie 的认证方式，可以作为现有 API Token 方式的补充或替代方案，特别适用于 API 限流或需要更稳定发布体验的场景。建议结合平台特性与自身使用场景，合理选择并完成相应配置。

## 附录
- 发布设置入口：/#/manage
- 帮助文档入口：README_zh_CN.md 中提供的帮助文档链接
- **新增** 语雀网页版平台规格说明：[openspec/specs/yuque-web-publishing/spec.md](file://openspec/specs/yuque-web-publishing/spec.md)