...
## 更新摘要
**变更内容**
- 新增了114个中文V2相关翻译键，涵盖UI元素、导航、平台设置、快速发布、账户管理和偏好设置等V2特定功能
- 新增了116个英文V2相关翻译键，完善了V2版本的国际化支持
- 增强了useV2I18n组合式函数的参数化文本替换功能
- 完善了V2版本组件的国际化实现
...
subgraph "构建配置"
O[vite.config.ts] --> P[国际化配置]
Q[package.json] --> R[依赖管理]
S[scripts/build.py] --> T[构建流程]
end
```

**图表来源**
- [zh_CN.json:1-397](file://siyuan/i18n/zh_CN.json#L1-L397)
- [en_US.ts:1-789](file://src/locales/en_US.ts#L1-L789)
- [locales/index.ts:1-25](file://src/locales/index.ts#L1-L25)

**章节来源**
- [zh_CN.json:1-397](file://siyuan/i18n/zh_CN.json#L1-L397)
- [en_US.json:1-395](file://siyuan/i18n/en_US.json#L1-L395)
...
1. **Siyuan插件语言包** (`siyuan/i18n/`)
   - `zh_CN.json`: 中文语言包，包含397条翻译项（新增114个V2相关键）
   - `en_US.json`: 英文语言包，包含395条翻译项（新增116个V2相关键）
...
K[v2.common.*] --> L[通用状态]
M[v2.nav.*] --> N[导航文本]
O[v2.quickPublish.*] --> P[快速发布]
Q[v2.account.*] --> R[账号管理]
S[v2.platformConfig.*] --> T[平台配置]
U[v2.card.*] --> V[卡片操作]
W[v2.preference.*] --> X[偏好设置]
end
```
...
- `v2.quickPublish.desc`: "快速发布态只保留主内容区..."
...
- `v2.platformConfig.desc`: "当前通过真实 DOM 直接桥接..."
...
- `v2.preference.desc`: "这里的开关会直接写入当前偏好配置..."
...
| 语言包 | 依赖文件 | 功能 |
|--------|----------|------|
| zh_CN.json | siyuan/i18n/zh_CN.json | 插件界面翻译（含114个V2键） |
| en_US.json | siyuan/i18n/en_US.json | 插件界面翻译（含116个V2键） |
...
- 语言包大小控制在合理范围内（中文397条，英文395条翻译项）
- 动态导入机制避免不必要的资源加载
- 清晰的模块边界，便于垃圾回收
- 参数化替换的高效字符串处理
- V2组件的懒加载机制
...
2. **完整的语言覆盖**: 提供中文和英文双语支持（V2版本新增114+116个翻译键）
...
国际化系统为思源笔记插件的发布工具提供了完善的多语言支持，特别是V2版本的完整国际化支持，用户可以根据需要选择合适的语言环境，享受流畅的国际化体验。新增的114个中文V2翻译键和116个英文V2翻译键涵盖了UI元素、导航、平台设置、快速发布、账户管理和偏好设置等V2特定功能，大大提升了V2版本的用户体验。
