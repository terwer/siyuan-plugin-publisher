...
## 更新摘要
**变更内容**
- V2App.vue重大更新，增强双视图系统和状态管理架构
- 新增useV2Settings组合式函数，提供完整的设置管理功能
- UnifiedWorkspaceShell.vue增加设置导航系统
- V2AccountList.vue和V2PlatformSelect.vue提供新的设置界面
- 改进发布状态管理和用户交互体验
...
Storage[本地存储]
WidgetUtils[WidgetPageUtils]
PublishUtils[usePublish.ts]
ConfigUtils[usePublishConfig.ts]
end
V2App --> QuickPub
V2App --> SettingsFn
QuickPub --> Store
...
SettingsFn --> DynConfig
QuickPub --> PrefStore
QuickPub --> WidgetUtils
QuickPub --> PublishUtils
QuickPub --> ConfigUtils
QuickPub --> SiyuanAPI
...
- [useV2Settings.ts:40-209](file://src/composables/v2/useV2Settings.ts#L40-L209)

### 状态管理架构

V2 应用的状态管理采用 Pinia 进行集中管理，新增了useV2Settings组合式函数来处理复杂的设置状态：
...
设置管理模块提供了灵活的平台配置和账户管理功能，新增了useV2Settings组合式函数来处理复杂的设置状态：
...
5. **状态管理增强**：新增useV2Settings组合式函数提供完整的设置管理功能
6. **双视图系统**：快速发布和设置视图之间的无缝切换

V2 系统不仅满足了当前的发布需求，还为未来的功能扩展奠定了坚实的技术基础。通过持续的迭代优化，V2 应用将成为思源笔记生态中最优秀的第三方发布工具之一。

**更新** 本版本重点增强了双视图系统的状态管理架构，新增了useV2Settings组合式函数来处理复杂的设置状态，改进了发布状态管理和用户交互体验。