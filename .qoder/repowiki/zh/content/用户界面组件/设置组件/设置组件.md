...
## 更新摘要
**变更内容**
- 新增V2AccountList.vue和V2PlatformSelect.vue组件，完善V2设置功能架构
- 更新V2设置系统的工作流，包括账号列表、平台选择和配置流程
- 完善V2设置组件的分层架构，体现Milestone 2的功能实现
- 新增V2设置状态管理和视图切换机制
- 更新设置组件的开发进度，反映Milestone 2的完成情况
...
- V2设置组件：V2AccountList、V2PlatformSelect
- 数据持久化、配置验证、热更新策略
...
## V2设置组件架构
V2设置组件是V2 UI系统的重要组成部分，专门负责账号设置、平台选择和配置管理。

### V2设置组件层次
- **账户列表组件**：V2AccountList负责展示和管理已配置的平台账号
- **平台选择组件**：V2PlatformSelect提供平台选择界面，支持预设平台的快速配置
- **设置状态管理**：useV2Settings提供完整的设置状态管理和业务逻辑
- **视图导航**：支持list/select/config三种视图模式的切换

```mermaid
graph TB
subgraph "V2设置组件"
VAL["V2AccountList.vue<br/>账号列表组件"]
VPS["V2PlatformSelect.vue<br/>平台选择组件"]
USV["useV2Settings.ts<br/>设置状态管理"]
end
subgraph "设置状态"
State["state.accountView<br/>视图状态"]
Items["state.accountItems<br/>账号列表"]
Select["selectablePlatforms<br/>可选平台"]
end
subgraph "业务流程"
List["loadAccountItems<br/>加载账号列表"]
SelectFlow["openPlatformSelect<br/>平台选择流程"]
ConfigFlow["openAccountConfig<br/>配置流程"]
Toggle["toggleAccountEnabled<br/>启用/禁用切换"]
Create["createAccountDraft<br/>创建账号草稿"]
end
VAL --> State
VPS --> Select
USV --> List
...
**图表来源**
- [V2AccountList.vue:16-55](file://src/components/v2/settings/V2AccountList.vue#L16-L55)
...
- [useV2Settings.ts:76-97](file://src/composables/v2/useV2Settings.ts#L76-L97)

**章节来源**
- [V2AccountList.vue:1-181](file://src/components/v2/settings/V2AccountList.vue#L1-L181)
- [V2PlatformSelect.vue:1-119](file://src/components/v2/settings/V2PlatformSelect.vue#L1-L119)
- [useV2Settings.ts:1-209](file://src/composables/v2/useV2Settings.ts#L1-L209)
...
- **V2设置层**：V2AccountList、V2PlatformSelect、useV2Settings提供现代化设置功能
...
subgraph "V2设置层"
VAL["V2AccountList.vue<br/>账号列表"]
VPS["V2PlatformSelect.vue<br/>平台选择"]
USV["useV2Settings.ts<br/>设置管理"]
end
subgraph "表单层"
...
#### V2AccountList组件
V2AccountList是V2设置系统的核心组件，负责展示和管理已配置的平台账号。

**核心功能**：
- 展示账号列表，包括平台图标、名称、唯一key和状态
- 支持添加新账号、配置现有账号、启用/禁用账号
- 提供简洁的卡片式布局，支持滚动查看更多账号
- 实时显示账号状态（启用/禁用、已授权/未授权）

```mermaid
flowchart TD
Start(["V2AccountList渲染"]) --> CheckItems{"是否有账号"}
CheckItems --> |无| ShowEmpty["显示空状态"]
CheckItems --> |有| RenderList["渲染账号列表"]
RenderList --> AccountItem["渲染单个账号卡片"]
AccountItem --> Actions["显示操作按钮"]
Actions --> Configure["配置按钮"]
Actions --> Toggle["启用/禁用按钮"]
Actions --> Delete["删除按钮(预留)"]
Configure --> EmitConfigure["触发配置事件"]
Toggle --> EmitToggle["触发切换事件"]
```

**图表来源**
- [V2AccountList.vue:16-55](file://src/components/v2/settings/V2AccountList.vue#L16-L55)
- [V2AccountList.vue:66-70](file://src/components/v2/settings/V2AccountList.vue#L66-L70)

**章节来源**
- [V2AccountList.vue:1-181](file://src/components/v2/settings/V2AccountList.vue#L1-L181)

#### V2PlatformSelect组件
V2PlatformSelect提供平台选择界面，支持用户从预设平台中选择合适的平台进行配置。

**核心功能**：
- 展示可选平台列表，包括平台图标和描述信息
- 支持一键选择平台，自动生成合适的平台key
- 提供清晰的平台选择引导，帮助用户快速配置
- 支持预设平台的快速创建和配置
...
#### useV2Settings状态管理
useV2Settings是V2设置系统的核心状态管理模块，提供完整的设置功能。

**核心状态**：
- `section`: 当前设置分组（account/picbed/preference）
- `accountView`: 账号设置视图状态（list/select/config）
- `accountItems`: 账号列表数据
- `selectedPlatformKey/Name`: 当前选中的平台信息
- `configMode`: 配置模式（create/edit）

**核心方法**：
- `loadAccountItems`: 加载并格式化账号列表
- `setSection`: 切换设置分组
- `openPlatformSelect/openAccountConfig`: 视图切换
- `toggleAccountEnabled`: 启用/禁用账号
- `createAccountDraft`: 创建账号草稿
...
**更新** 本阶段已实现V2AccountList和V2PlatformSelect组件，完成账号列表与平台选择的核心功能
...
- **状态管理优化**：useV2QuickPublish和useV2Settings使用响应式状态，避免不必要的重新渲染
- **样式隔离**：V2样式使用命名空间，避免与传统样式冲突
- **组件优化**：V2AccountList使用虚拟滚动，支持大量账号的高效渲染
...
- **账号列表为空**：检查useV2Settings的loadAccountItems方法，确认配置加载正常
...
- **功能完善**：新增V2AccountList和V2PlatformSelect组件，完成Milestone 2的核心功能
