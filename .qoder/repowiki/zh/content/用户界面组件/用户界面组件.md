...
- 添加 V2 UI 迁移规范和里程碑管理
- 新增 V2PlatformCard.vue 和 useV2QuickPublish.ts 组件分析
- 更新组件通信机制以包含 V2 架构的工厂函数模式
- **新增** V2 图片上传警告UI显示功能：当状态为success_with_warnings时，现在会显示详细的图片上传错误信息面板，包括可展开的详细错误列表
...
- [V2App.vue:1-588](file://src/components/v2/V2App.vue#L1-L588)
...
- [variables.styl:1-120](file://src/assets/v2/variables.styl#L1-L120)
- [vite.v2.config.ts:1-137](file://vite.v2.config.ts#L1-L137)
- [v2Host.ts:1-108](file://siyuan/v2/v2Host.ts#L1-L108)
- [V2PlatformCard.vue:1-84](file://src/components/v2/publish/V2PlatformCard.vue#L1-L84)
- [useV2QuickPublish.ts:1-323](file://src/composables/v2/useV2QuickPublish.ts#L1-L323)
...
- **新增** V2 应用组件
  ...
  - V2 应用宿主：V2Host
  - V2 平台卡片：V2PlatformCard.vue
  - V2 快速发布组合式函数：useV2QuickPublish.ts
...
- **V2PlatformCard.vue**：平台卡片组件，展示平台状态和图标
- **useV2QuickPublish.ts**：组合式函数，管理快速发布状态和数据流
...
BASE --> V2PLATFORMCARD["V2PlatformCard.vue<br/>平台卡片样式"]
V2APP --> COMPONENTS["组件样式"]
UWS --> LAYOUT["布局样式"]
V2PLATFORMCARD --> CARDSTYLE["卡片样式"]
```

**图表来源**
- [variables.styl:1-120](file://src/assets/v2/variables.styl#L1-L120)
- [base.styl:1-433](file://src/assets/v2/base.styl#L1-L433)
- [V2App.vue:372-588](file://src/components/v2/V2App.vue#L372-L588)
- [UnifiedWorkspaceShell.vue:1-40](file://src/components/v2/layout/UnifiedWorkspaceShell.vue#L1-L40)
- [V2PlatformCard.vue:26-84](file://src/components/v2/publish/V2PlatformCard.vue#L26-L84)
...
### V2 快速发布组合式函数
`useV2QuickPublish.ts` 管理 V2 架构中的快速发布状态，包括平台数据获取、文档上下文读取和状态管理：
...
### V2 图片上传警告UI显示功能
**更新** V2 应用现在支持图片上传警告UI显示功能。当发布状态为 `success_with_warnings` 时，会在发布状态面板中显示详细的图片上传错误信息面板，包括可展开的详细错误列表。

#### 功能实现机制
- **状态检测**：在 `useV2QuickPublish.ts` 中，发布成功后检查 `result.errMsg` 是否存在
- **状态设置**：如果存在错误信息，将发布状态设置为 `success_with_warnings` 而非普通 `success`
- **UI 显示**：在 `V2App.vue` 中，当状态为 `success_with_warnings` 且存在错误信息时，显示警告面板
- **错误信息展示**：使用 `<pre>` 标签保持错误信息的原始格式，支持换行和缩进
...
#### 样式设计
- **警告面板**：使用 `$syp-status-warning-deep-bg` 背景色和 `$syp-status-warning-border` 边框色
- **标题样式**：使用 `$syp-warning` 主色调，突出警告性质
- **文本样式**：使用 `#b37400` 橙色系颜色，保持与警告主题一致
- **格式保持**：通过 `white-space: pre-wrap` 保持错误信息的原始格式
...
- [variables.styl:28-43](file://src/assets/v2/variables.styl#L28-L43)
- [base.styl:479-495](file://src/assets/v2/base.styl#L479-L495)
...
- 视图模式：
  ...
  - 设置模式：显示完整的设置界面
- **新增**：Milestone 2 完整骨架实现，包括品牌区、导航区、详情区的占位符
...
#### V2 平台卡片组件（V2PlatformCard.vue）
- 作用：展示单个平台的信息，包括图标、名称和发布状态
- 特性：
  - 支持自定义图标或使用平台名称首字母作为占位符
  - 动态状态指示（已发布/未发布）
  - 响应式设计和悬停效果
- **新增**：专门用于 V2 快速发布界面的平台展示组件

**章节来源**
- [V2PlatformCard.vue:1-84](file://src/components/v2/publish/V2PlatformCard.vue#L1-L84)

#### V2 快速发布组合式函数（useV2QuickPublish.ts）
- 作用：管理 V2 架构中的快速发布状态和数据流
- 功能：
  - 获取当前文档上下文（pageId、标题）
  - 加载已启用且已授权的平台列表
  - 检测每个平台的发布状态
...
- [v2Host.ts:1-108](file://siyuan/v2/v2Host.ts#L1-L108)
- [vite.v2.config.ts:1-137](file://vite.v2.config.ts#L1-L137)
- [ui-v2-migration/spec.md:85-116](file://openspec/changes/refactor-ui-v2-foundation/specs/ui-v2-migration/spec.md#L85-L116)
- [tasks.md:18-24](file://openspec/changes/refactor-ui-v2-foundation/tasks.md#L18-L24)
...
- 组件耦合
  ...
  - **V2 组件**：V2App.vue 作为根组件，依赖 UnifiedWorkspaceShell.vue、V2PlatformCard.vue 和样式系统
  - **V2 宿主**：V2Host 依赖 V2App.vue 和思源原生 Menu API
  - **V2 组合式函数**：useV2QuickPublish.ts 依赖多个存储和工具函数
  - **V2 图片上传警告**：依赖 useV2QuickPublish.ts 的错误状态检测和 V2App.vue 的 UI 展示
- 外部依赖
...
- 可能的循环依赖
  ...
  - **V2 宿主**：通过接口抽象避免循环依赖
  - **V2 组合式函数**：使用响应式状态管理，避免循环引用
  - **V2 图片上传警告**：通过状态检测和条件渲染避免循环依赖
...
V2APP --> V2PLATFORMCARD["V2PlatformCard.vue"]
V2HOST["V2Host"] --> V2APP
VITE["vite.v2.config.ts"] --> V2APP
COMPOSABLE["useV2QuickPublish.ts"] --> V2APP
WARNING["图片上传警告"] --> V2APP
WARNING --> COMPOSABLE
```
...
- [SinglePublish.vue:10-21](file://src/pages/SinglePublish.vue#L10-L21)
- [BatchPublish.vue:10-21](file://src/pages/BatchPublish.vue#L10-L21)
- [V2App.vue:104-111](file://src/components/v2/V2App.vue#L104-L111)
- [UnifiedWorkspaceShell.vue:1-40](file://src/components/v2/layout/UnifiedWorkspaceShell.vue#L1-L40)
- [v2Host.ts:1-108](file://siyuan/v2/v2Host.ts#L1-L108)
- [useV2QuickPublish.ts:1-323](file://src/composables/v2/useV2QuickPublish.ts#L1-L323)
...
- 渲染优化
  ...
  - **V2 架构**：响应式布局优化，适应不同设备性能
  - **V2 图片上传警告**：使用条件渲染，仅在状态为 success_with_warnings 时才渲染警告面板
- 异步与并发
...
- **V2 性能优化**
  ...
  - **V2 架构**：组合式函数使用响应式状态，避免不必要的重新渲染
  - **V2 图片上传警告**：错误信息使用 `<pre>` 标签保持格式，避免额外的字符串处理开销
...
- **V2 架构故障排查**
  ...
  - **V2 架构**：平台卡片显示异常：检查平台图标和状态数据
  - **V2 架构**：快速发布状态异常：检查 useV2QuickPublish.ts 的数据获取逻辑
- **V2 UI 迁移故障排查**
  - **V2 架构**：Milestone 任务未按顺序执行：检查任务清单状态
  - **V2 架构**：样式污染：确认构建配置中的样式隔离机制
- **V2 图片上传警告故障排查**
  - **V2 图片上传警告**：检查 `useV2QuickPublish.ts` 中的错误状态检测逻辑
  - **V2 图片上传警告**：确认 `V2App.vue` 中的条件渲染逻辑是否正确
  - **V2 图片上传警告**：检查国际化文本 `v2.quickPublish.warning.imageUploadFailed` 是否正确配置
  - **V2 图片上传警告**：验证错误信息格式是否正确，确保 `<pre>` 标签正确显示
...
**新增的图片上传警告UI显示功能进一步提升了用户体验**，当发布过程中出现图片上传错误时，用户能够清楚地看到详细的错误信息，便于问题诊断和解决。该功能通过状态检测、条件渲染和专门的UI设计，实现了与整体架构的无缝集成。
...
- 父子通信
  ...
  - **V2 架构**：组合式函数使用响应式状态，提供类型安全的状态管理
  - **V2 图片上传警告**：通过状态检测和条件渲染实现非侵入式错误提示
- 事件命名
...
- 响应式布局
  ...
  - **V2 架构**：统一工作壳支持移动端自适应
  - **V2 图片上传警告**：在小屏设备上保持错误信息的可读性
- **V2 架构**：样式隔离
  - 所有样式都包裹在 `.syp-v2` 命名空间下
  - 确保与思源笔记样式完全隔离
  - 支持动态主题切换
  - **V2 图片上传警告**：使用独立的样式类名避免与其他组件冲突
...
- **V2 架构扩展**
  ...
  - **V2 架构**：状态管理：通过 useV2QuickPublish.ts 扩展新的状态逻辑
- **V2 UI 迁移扩展**
  - 里程碑管理：遵循任务清单的顺序执行原则
  - 规范遵循：严格遵守 V2 UI 迁移规范的各项要求
  - 回滚机制：确保新功能不影响现有 Legacy UI 的稳定性
- **V2 图片上传警告扩展**
  - 错误信息格式化：可以在适配器层提供更详细的错误信息格式
  - 多语言支持：为不同语言环境提供相应的警告信息文本
  - 用户交互：可以添加"复制错误信息"等功能按钮
...
- **V2 架构**：数据流管理：通过 useV2QuickPublish.ts 统一管理状态
- **V2 图片上传警告**：错误状态检测与UI渲染的完整流程

**章节来源**
- [vite.v2.config.ts:1-137](file://vite.v2.config.ts#L1-L137)
- [createV2App.ts:15-36](file://src/v2/createV2App.ts#L15-L36)
- [V2App.vue:104](file://src/components/v2/V2App.vue#L104)
