...
- [proposal.md](file://openspec/changes/refactor-ui-v2-foundation/proposal.md)
...
- [SKILL.md](file://.codex/skills/openspec-propose/SKILL.md)
- [SKILL.md](file://.codex/skills/openspec-apply-change/SKILL.md)
- [SKILL.md](file://.codex/skills/openspec-archive-change/SKILL.md)
- [SKILL.md](file://.codex/skills/openspec-explore/SKILL.md)
- [explore.md](file://.claude/commands/opsx/explore.md)
- [archive.md](file://.claude/commands/opsx/archive.md)
</cite>
...
**更新** 从传统OpenSpec工作流向OPSX实验性工作流迁移，引入新的配置框架和命令系统
...
- **新增** OPSX实验性工作流：通过新的配置框架和命令系统，提供更高效的变更管理体验。
...
subgraph "OPSX配置框架"
CFG --> CFG_schema["schema: spec-driven"]
CFG --> CFG_rules["per-artifact rules"]
end
AG --> C
P --> S
P --> C
...
- [config.yaml:1-21](file://openspec/config.yaml#L1-21)
...
### 组件E：OPSX实验性工作流配置框架
**新增** OPSX实验性工作流引入新的配置框架，提供更灵活的项目管理和工作流定制：

- config.yaml：项目配置文件，定义工作流模式和规则
- schema：指定工作流模式（如 spec-driven）
- per-artifact rules：为特定工件类型定义自定义规则
- 项目上下文：添加技术栈、约定、样式指南等背景信息

**章节来源**
- [config.yaml:1-21](file://openspec/config.yaml#L1-21)

### 组件F：OPSX实验性工作流命令系统
**新增** OPSX实验性工作流提供全新的命令系统和技能接口：

- openspec-propose：一键生成完整的变更提案（proposal、design、tasks）
- openspec-apply-change：实现变更任务的智能助手
- openspec-archive-change：智能归档已完成的变更
- openspec-explore：探索模式，用于思考和澄清需求
- OPSX命令：/opsx:explore、/opsx:archive 等实验性命令

**章节来源**
- [SKILL.md:1-111](file://.codex/skills/openspec-propose/SKILL.md#L1-111)
- [SKILL.md:1-157](file://.codex/skills/openspec-apply-change/SKILL.md#L1-157)
- [SKILL.md:1-115](file://.codex/skills/openspec-archive-change/SKILL.md#L1-115)
- [SKILL.md:1-289](file://.codex/skills/openspec-explore/SKILL.md#L1-289)
- [explore.md:1-22](file://.claude/commands/opsx/explore.md#L1-22)
- [archive.md:1-10](file://.claude/commands/opsx/archive.md#L1-10)
...
- **新增** config.yaml 为OPSX实验性工作流提供配置依赖。

**更新** OPSX实验性工作流引入新的依赖关系
...
- **新增** OPSX实验性工作流：通过智能配置和命令系统，提高变更管理效率，减少手动操作。
...
- **新增** OPSX实验性工作流故障排除：
  - "配置文件错误"：检查 config.yaml 格式和语法
  - "命令执行失败"：确认安装了正确的OPSX工具链
  - "技能接口异常"：验证技能文件完整性和权限设置
...
## 结论
OpenSpec 通过"规范即真相、提案即计划、归档即历史"的三段式结构，结合严格的文件格式与命令行工具，实现了从"想法"到"落地"的闭环管理。配合 AGENTS.md 的角色分工与协作原则，既能发挥 AI 助手在规范化与自动化方面的优势，又能确保人类作者在方向把控与业务理解上的主导地位。

**更新** OPSX实验性工作流的引入，通过新的配置框架和命令系统，进一步提升了变更管理的效率和智能化水平。新的工作流不仅保持了原有OpenSpec的核心理念，还通过智能配置、自动化技能接口和实验性命令，为开发者提供了更现代化的变更管理体验。

对于新加入的团队成员，建议先通读 AGENTS.md 与 project.md，再参考具体示例文件（如 proposal.md、tasks.md、design.md、spec.md），最后了解OPSX实验性工作流的新特性，快速上手高效的变更管理工作。
...
### 使用指南：OPSX实验性工作流操作
**新增** OPSX实验性工作流提供了全新的操作方式：

- **提案创建**：使用 `/opsx:propose` 命令快速创建完整的变更提案
- **任务实现**：使用 `/opsx:apply` 命令实现变更任务
- **变更归档**：使用 `/opsx:archive` 命令归档已完成的变更
- **探索模式**：使用 `/opsx:explore` 命令进行深度思考和问题探索
...
- **新增** 示例3：UI V2 全生命周期重构
  - 提案：openspec/changes/refactor-ui-v2-foundation/proposal.md
  - 任务：openspec/changes/refactor-ui-v2-foundation/tasks.md
  - 设计：openspec/changes/refactor-ui-v2-foundation/design.md
  - 规范：openspec/changes/refactor-ui-v2-foundation/specs/ui-v2-migration/spec.md
...
- [proposal.md:1-46](file://openspec/changes/refactor-ui-v2-foundation/proposal.md#L1-46)
- [tasks.md:1-80](file://openspec/changes/refactor-ui-v2-foundation/tasks.md#L1-80)
- [design.md:1-807](file://openspec/changes/refactor-ui-v2-foundation/design.md#L1-807)
- [spec.md:1-236](file://openspec/changes/refactor-ui-v2-foundation/specs/ui-v2-migration/spec.md#L1-236)