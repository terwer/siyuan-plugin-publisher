...
```text
feat!: migrate the UI architecture from iframe SPA to native DOM mounting
```

## Commit Body

```text
BREAKING CHANGE: Introduce the unified workspace and complete the UI architecture migration from iframe-based SPA hosting to native DOM mounting.
...
- Establish real DOM mounting as the sole foundation for all UI capabilities
- Deprecate iframe-based page hosting for all new UI features
...
Migration Strategy:
- Introduce the 「旧界面开关」 feature toggle to enable gradual rollout with full rollback capability
...
- M0: Establish entry governance, unified preference loading, and the host
...
- The build chain, including `vite.config.ts` for isolated UI validation

Migration Guide:
- Enable "Use new UI (experimental)" in preference settings to opt in to the new UI
