...
平台帮助系统和 TourGuide 框架已经上线，但平台配置覆盖仍不均衡：博客园等少数平台有完整的 `summary / fields / faq / tour`，而大多数 T1 平台仍停留在 `remaining-t1.ts` 的 helpUrl 级别。随着 `platform-verification` 持续推进，每个平台验证中暴露出的配置坑点、授权步骤和发布注意事项需要及时沉淀到帮助与引导中，避免验证完成后经验丢失。
