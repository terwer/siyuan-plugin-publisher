...
- 8.5 已基于 Chrome Debug 取证完成根因修复：按 id 读取详情必须使用 `GET /api/docs/{id}?book_id={bookId}`，裸 `GET /api/docs/{id}` 返回 404；更新响应正文在 `body`，详情回读正文在 `content`。已补充单测并通过 `pnpm exec vitest run src/adaptors/web/yuqueweb/YuquewebWebAdaptor.spec.ts src/composables/usePublish.spec.ts --reporter=dot` 与 `pnpm exec vue-tsc --noEmit`。后续仍建议用户按 8.5 人工流程复测 UI 全链路。
- 8.6 当前人工验证失败：包含本地图片的文档发布后图片未同步，详情提示为语雀图片上传失败。
