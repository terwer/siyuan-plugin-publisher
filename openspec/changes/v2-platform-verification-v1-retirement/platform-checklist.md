...
| 7 | Hugo | `github_Hugo` | `Github_Hugo` | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | 代码层准备就绪（2026-09-03，图片策略 2026-09-04 确认）：Hugo 图片链路已按官方规范核验正确——`imageStorePath=static/images` + `imageLinkPath=images` → 文章引用绝对路径 `/images/<名>`（Hugo 构建把 `static/` 原样复制到站点根，构建产物中该路径可靠显示，符合官方推荐的 static/ 契约与作者本地仓库`hexo-image-processing-zftcmx.md` 的 `/images/…png` 惯例）；converter front matter 不写 `draft`（官方默认 false=发布），无隐藏草稿风险。**SOP §3 help/tour/doc ✅（代码+文档部分）**：新增 `github-hugo.ts`（pageId `github_Hugo`，helpUrl+summary+fields+faq4+tour9，字段/图片/permalink 与真实 Hugo 语义一致）+ `docs/draft/platforms/github-hugo.md`（顶部 TODO 占位，含 static 目录规范与「推送后需触发 hugo 构建才上线」说明），注册进 `/pages/index.ts`、从 `remaining-t1` 移出并纳入 `verifiedConfigs`（registry.spec 18 项全绿、build:v2 通过）。**宿主手验六格+HelpPanel/TourGuide 实测待执行** |
...
| 2026-08-25 | #6 Hexo `github_Hexo` **V2 全链路六格 + 帮助引导 ✅（Electron 宿主，test 工作空间/9222/terwer/hexo-blog）**：V2C（username/repo/branch/PAT Token，「验证」通过，「配置已保存并验证通过」，账号「运行中/已启用」）；Pub → repo `source/_posts/掘金-V2-验证测试-更新.md`（提交 `bdf5415`）；Upd 点「更新」→ 新提交 `3b9692e`；Img 图床切「**当前平台**」重发，cat 图上传 `source/images/…jpg`，.md 引用 `/images/…jpg`（此前误记 `/source/images/…jpg`，已于本次修复更正）；查看 blob URL HTTP 200；Del 删除 → repo .md 移除（404），提交 `3cc1abb`。新增 `common-github-hexo.ts` help 配置 + `docs/draft/platforms/common-github-hexo.md`，注册进 `/pages/index.ts`、移出 `remaining-t1`、纳入 verifiedConfigs（registry 18 项绿）。**依赖修复**：`pnpm install` 将 `zhi-blog-api` 由 1.80.0 升到 **1.82.0**（此前 node_modules 落后，致 `BlogConfig.isCorsProxy` 类型缺失、`vue-tsc` 失败）；`build:v2` 通过。T1 小结更新为 17 个全链路 ✅ |
| 2026-08-25 | #6 Hexo 图片链接修复：用户反馈「build 后图片 404」。核对本地 `hexo-blog` 源码（`_config.yml` `post_asset_folder:false`/`source_dir:source`/`public_dir:docs`，作者自建 `hexo-图片-2.md`/`hexo-图片处理.md` 均引用 `/images/…png`）与站点实况（`/images/cat-…jpg` HTTP 200、`/source/images/cat-…jpg` 404、作者 `/images/…png` 页 200）→ 根因 `HexoConfig.imageLinkPath` 误设为 `source/images`，文章生成 `/source/images/…jpg`（构建后 404），应改用 `images` → `/images/…jpg`。修复 commit `39323796`（`imageLinkPath="images"`，`imageStorePath` 保持 `source/images`）+ `hexoImagePath.spec.ts`；build:v2 通过。宿主把账号配置页「图片访问链接」改 `images` 并重发 → repo 引用已改为 `/images/cat-…jpg`，**站点实况确认**：live 文章页 `<img src="/images/cat-…jpg">` 且该资源 HTTP 200（原 `/source/images/…jpg` 404 已消失）。**存量账号需在配置页更新该字段（字段可编辑，默认仅对新账号生效）** |
