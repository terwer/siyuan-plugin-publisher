...
> 关联主线：`platform-verification`（应用功能完备后可推进三版本后移除 iframe）
> 方案细节：`design.md`、`.planning/2026-08-24-publish-loop/`
> 交互原型（已批准 ~90%）：`tmp/v2-article-manage-real.html`
> **关键验收 = 既有发布组件解耦后旧界面回归无退化 + 插件宿主手验通过**

## 0. 治理（本提案）

- [x] 0.1 创建 OpenSpec 变更 `publish-loop`
- [x] 0.2 确认方案B（抽公共、放弃整体桥接A）
- [x] 0.3 确认新界面禁用 vue-router（五视图状态机）
- [x] 0.4 确认公共组件彻底去耦合（去 router/store/i18n，数据+动作注入，旧界面与新界面完全独立）
- [x] 0.5 确认交互原型（单发/批发/闪发 = 次级滑入面板、列表仍在背后、平台 chip 悬停/点击）
- [x] 0.6 撰写 proposal / design / specs / tasks

## 1. Phase 1 — 复用共享发布引擎 + 解耦既有发布外壳

> **关键验收**：旧界面单发/批发的行为回归无退化（只把既有发布外壳的 `useRoute/useRouter` 改为 props+emit）。

- [x] 1.0 确认「公共部分」= 共享发布引擎 `usePublish`（`doSinglePublish`/`assignInitAttrs`/批量循环）+ **既有成熟表单组件链**（`SinglePublishDoPublish` / `BatchPublishIndex` / 全套 form 子组件）供界面各处复用
...
- [x] 1.4 解耦 `BackPage.vue`：去顶层 `useRoute/useRouter`，改 `inject(routerKey/routeLocationKey)` 容错（新界面无 vue-router 不崩、旧界面行为不变），新增可选 `showBack` prop
- [x] 1.5 新增旧界面包装页接管路由取参：`pages/SinglePublishDoPublish.vue`（`/publish/singlePublish/doPublish/:key/:id`）、`pages/QuickSelect.vue`（`/publish/quickSelect`）；`routeConfig.ts` 相应改装
- [x] 1.6 **旧界面回归对照**：`pnpm build`（插件 + 打包）通过 + `pnpm vitest run` 56 文件 / 276 用例通过（无退化）
...
## 3. Phase 3 — single_publish / batch_publish 视图（复用既有成熟实现）

- [x] 3.1 `SinglePublish.vue` 重写为**复用壳**：既有两步流程（`SinglePublishSelectPlatform` 选平台 → `SinglePublishDoPublish` 详表单），预设平台→直接详表单，轻壳提供返回+标题
- [x] 3.2 `BatchPublish.vue` 重写为**复用壳**：直接复用 `BatchPublishIndex`（本身无 vue-router）
- [x] 3.3 `V2QuickPublishGrid.vue`：闪发快速平台格（逐卡发布/更新），复用 `useQuickPublish`
- [x] 3.4 回调内**绝不 `router.push`**（全程 `currentView` 状态机）
- [x] 3.5 删除精简引擎 `useSinglePublish.ts`/`useBatchPublish.ts` 及其 spec（不再重写，改为复用既有成熟实现）
- [x] 3.6 思源 AI 配置重构适配 + 界面各处共用 AI 设置组件：`usePreferenceSettingStore` 读 `config.ai.providers[]`（默认选 agent.modelId，提供方无 apiKey 则回退到带密钥的启用 provider）、`AiSetting.vue` 升级为共用组件（按 provider 分组的 `el-select` 模型选择器）、设置页新增同级 AI tab（`ai` section + nav），`PublishPreferenceCfg` 记录 `experimentalSisyuanAiActiveModelId`

## 4. Phase 4 — 入口接线

- [x] 4.1 quick_publish 头部「详细发布」文字 button（`openSinglePublishForCurrent` → `single_publish` 全视图，复用既有两步流程）
...
- [x] 4.5 次级面板交互：右侧滑入、列表仍在背后（遮罩压暗不隐藏）、←/遮罩/Esc 关闭（`App.vue` 管理面板 + `handleWindowKeydown`）
