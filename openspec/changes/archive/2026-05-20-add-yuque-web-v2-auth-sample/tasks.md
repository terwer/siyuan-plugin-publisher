...
- [x] 5.3 验证包含本地图片的发布会复用现有发布主链路完成图片识别、上传和 URL 替换
...
- [x] 8.6 发布包含本地图片的文档，确认语雀侧图片可访问且正文 URL 正确替换
...
- 8.5 已由用户确认测试通过。
- 2026-05-20 用户 V2 宿主复测通过：`fix-v2-yuque-image-upload-host-dialog` 修复表单上传与 `forceProxy` 解耦（插件内 `zhi-formdata-fetch`）；验收文档 https://www.yuque.com/terwer/public/feature-testing-2uu6u 、https://www.yuque.com/terwer/public/v20-test-only-zebdlv
- 8.10 根因（2026-05-20）：`common_Yuque` 的 429 为语雀 **Open API / Token 需专业会员** 及免费配额限制，未续费即 429；详见 `verification-2026-05-20-yuque-api-membership.md` 与 https://www.yuque.com/about/price#personal
