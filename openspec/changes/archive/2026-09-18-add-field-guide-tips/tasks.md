...
- [x] 4.2 locales 的 `setting.blog.*.tip` 共享串保持不动，确认 V1 界面文案零变化。本 change 全程**未改动 `setting.blog.*.tip` 共享串**（`git log --since=2026-09-07 -p -- siyuan/i18n` 中无任何 `setting.blog.*.tip` 增删），V1 路径与文案零改动。注：4.4 为新增的平台入口补了一个**平台描述键**（`setting.platform.metaweblog.metaweblog.desc`）并同步 4 处，属新增键、不在共享 `tip` 串之列。
...
- [x] 4.4 补齐 `metaweblog_Metaweblog`（Metaweblog 通用）并使其可核验（2026-09-18）。**已完成**：按 4.1/4.3 同一口径新建 `src/helpConfigs/pages/platform-config/metaweblog-metaweblog.ts`（summary + fields 7 + faq 5 + tour 8），纳入 `registry.spec.ts` 的 `verifiedConfigs` 与 `verifiedPlatformRows.ts` 冻结行，补 `docs/draft/platforms/metaweblog-metaweblog.md`，`remaining-t1` 至此**清空**。该站原先未登记在 `src/platforms/pre.ts`（`metaweblogCfg` 仅 Cnblogs / Typecho / Jvue），选择器不提供入口因而**无法核验**；按用户决策登记为**用户可选入口**：`metaweblogCfg` 末位（排在博客园 / Typecho / Jvue 之后作兜底）、中性订阅源图标、i18n 新增 `setting.platform.metaweblog.metaweblog.desc` 并同步 4 处。宿主验证：选择器 MetaWeblog 组 **4 项**、配置页 `[data-syp-field-guide]`=**7** 且字段顺序与冻结集一致、5 个文本框占位均为示例值（`https://your-blog.com`、`https://your-blog.com/xmlrpc.php`、`your-blog-name`、`your-blog-password`、`/?p=[postid]`）、7 条 ⓘ 各自专属、帮助浮层 summary + FAQ 5 条、引导 **8/8**；核验用临时账号按**精确 platformKey** 删除、账号数复原 **32**。回归：`pnpm vitest run` **67 文件 / 319 测试**全绿、`pnpm build:v2` 通过、`openspec validate --strict` 通过。
