...
通过桥接方式在 V2 中提供批量分发能力，复用现有 `BatchPublishIndex.vue` 实现。
...
通过桥接方式在 V2 中提供文章管理仪表盘能力，复用现有 `Admin.vue` 实现。

#### 范围

- 在 V2 Host 中添加文章管理仪表盘入口
- 桥接现有 `Admin.vue` 组件
...
在 V2 快速发布页面整合详细发布模式，支持摘要、标签、分类等高级字段设置。优先尝试桥接现有详细模式组件，若桥接不可行则原生实现。

#### 范围

- 在 V2 快速发布页面（V2PlatformCard）添加"详细设置"入口
...
- 用户可在 V2 快速发布页面打开详细设置
- 详细设置支持摘要、标签、分类等字段编辑
- 设置完成后发布功能正常工作
- 若采用桥接方案，不影响 V1 详细模式功能
- 若采用原生实现，需说明桥接不可行的原因
...
| **桥接依赖功能** | V2 已桥接，但底层仍依赖旧组件 | 26 个平台的配置表单（`V2PlatformConfigBridge`）、批量分发（`BatchPublishIndex.vue` 桥接）、文章管理仪表盘（`Admin.vue` 桥接） |
| **未迁移功能** | V2 尚无等价实现，必须保留 | 常规发布、AI 聊天、测试页面、关于页面 |
...
1. **功能缺失**：V2 尚未实现该 SPA 页面的等价功能（如常规发布、AI 聊天）
2. **桥接未完成**：该 SPA 承载的功能被 V2 部分桥接，但桥接范围未覆盖全部子类型（如平台配置桥接目前覆盖 26 个子类型，若新增平台尚未桥接；批量分发和文章管理仪表盘当前为桥接状态，在 M6/M8 评估是否原生重写）
