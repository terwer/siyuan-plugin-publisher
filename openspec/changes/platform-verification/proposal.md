...
界面已具备快速发布与平台配置桥接，但缺少**唯一、可追踪**的全平台验收源头；旧 checklist 散落在 `.qoder/plans/`，与 `.planning/` 并行易造成进度分裂。需要在 OpenSpec 中集中管理「逐平台验证 → 修缺陷 → 旧界面废弃 → 三版本后移除 iframe」这条确定性长周期任务。
...
- `platform-verification`: 定义全平台验收分层、检查项、进度表维护规则与旧界面退役门禁。

### Modified Capabilities

- `ui-migration`: 补充「全平台验收完成」作为 iframe 物理删除的前置条件，与 Gate A–D 对齐。

## Impact

- 文档与流程：`openspec/changes/platform-verification/*`（主表 + tasks）。
- 实现工作：按 checklist 失败项触发的各平台适配器/界面桥接修复（不在本提案一次性改完）。
- 删除：`.qoder/plans/全量平台测试Checklist_0578ad66.md`（已移除）。
- 不改动：未验收平台的运行时行为；旧界面代码在 Gate D 之前保持可用。
