...
系统 SHALL 为所有网页 Cookie 授权平台提供统一退出/清除授权能力，优先执行平台适配器提供的动作化退出，并在成功后清理本地授权状态。界面层 SHALL 只调用统一退出能力，不得为语雀、CSDN、知乎或其他网页平台复制退出流程。
...
- **WHEN** 用户在 CSDN 或知乎配置页触发退出/清除授权
- **THEN** 界面 SHALL 调用统一网页 Cookie 退出能力
