# v2-shared-article-management Specification

## Purpose
为 V2 原生 DOM 应用提供文章管理视图：复用无 router 依赖的共享文章管理组件（与 V1 同一源，不两张皮），经面板头部房子图标进入，全部管理动作（快发/单发/批发/查看/图床）以 V2 原生实现，不向 V1 兜底。
## Requirements
