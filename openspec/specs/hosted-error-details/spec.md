# hosted-error-details Specification

## Purpose

定义 在思源宿主弹窗内展示错误详情的局部、紧凑、可复制、宿主安全交互能力，覆盖快速发布失败与平台配置验证失败等需要保留真实诊断的场景。
## Requirements
### Requirement: 错误详情 SHALL 在宿主插件容器内展示
系统 SHALL 在 思源插件宿主弹窗内部展示错误详情，不得依赖默认挂载到 `document.body` 的全局弹窗造成遮罩、定位或尺寸脱离 `.syp-app` 容器。

#### Scenario: 用户查看快速发布失败详情
- **WHEN** 用户在快速发布状态卡片点击“查看详情”
- **THEN** 系统 SHALL 在 `.syp-app` 容器内部显示错误详情交互
- **AND** 详情交互 SHALL 不创建覆盖整个思源窗口的错位遮罩
- **AND** 详情交互 SHALL 保持在当前插件面板可视区域内

#### Scenario: 用户查看平台配置验证失败详情
- **WHEN** 用户在 账号「发布设置」桥接页点击验证且失败，并点击「查看详情」
- **THEN** 系统 SHALL 在 `.syp-app` 容器内部显示错误详情交互（与快速发布共用或等价组件）
- **AND** 详情交互 SHALL 不创建覆盖整个思源窗口的错位遮罩
- **AND** 详情交互 SHALL 保持在当前插件面板可视区域内

#### Scenario: 运行在思源宿主弹窗中
- **WHEN** 面板被思源以插件弹窗或浮层形式承载
- **THEN** 错误详情 SHALL 使用 界面自身的层级、圆角、阴影、间距和紧凑视觉
- **AND** 错误详情 SHALL NOT 破坏宿主弹窗的滚动、关闭、拖动或焦点行为

### Requirement: 错误详情 SHALL 同时支持友好提示和真实诊断
...
### Requirement: 快速发布错误摘要 SHALL 从共用层提取业务原因
系统 SHALL 在快速发布共用层从错误文本、错误对象或发布结果中提取面向用户的短错误摘要。提取逻辑 MUST 适用于所有平台，不得为 CSDN、知乎、语雀或其他平台编写专属分支。
...
### Requirement: 快速发布详情 SHALL 保留真实诊断
系统 SHALL 将快速发布失败的短摘要与真实诊断详情分离。页面状态卡显示短摘要，错误详情面板显示脱敏后的完整错误、traceId、HTTP 状态、响应摘要和调用堆栈（如存在）。
