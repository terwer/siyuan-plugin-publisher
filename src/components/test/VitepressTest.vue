...
const wordpressHandleApi = async () => {
  ...
  try {
    ...
    switch (methodOption.value) {
      case METHOD_GET_USERS_BLOGS: {
        const vitepressCfg = {}
        const vitepressApiAdaptor = {}
        const vitepressApi = Utils.blogApi(appInstance, vitepressApiAdaptor)
        const result = await vitepressApi.getUsersBlogs()
        logMessage.value = JSON.stringify(result)
        logger.info("vitepress users blogs=>", result)
        break
      }
      case METHOD_GET_RECENT_POSTS_COUNT: {
        break
