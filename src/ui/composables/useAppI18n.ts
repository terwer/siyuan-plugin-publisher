...
export const I18N_FALLBACK_KEY: InjectionKey<I18nFallback> = Symbol("publisher-i18n-fallback")

export const useAppI18n = () => {
  const { locale, messages } = useI18n()
  const fallback = inject(I18N_FALLBACK_KEY, undefined)
  ...
  const resolve = (key: string, params?: Record<string, I18nValue>) => {
  ...
  return {
    t: (key: string, params?: Record<string, I18nValue>) => resolve(key, params).text,
    inspect: resolve,
    locale,
  }
}
