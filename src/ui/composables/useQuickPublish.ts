...
import { sortQuickPublish } from "~/src/ui/composables/platformOrdering.ts"
import {
  buildQuickPublishCaughtErrorText,
  buildQuickPublishErrorText,
  sanitizeQuickPublishText,
} from "~/src/ui/composables/quickPublishErrorText.ts"
import { notifyQuickPublishResult } from "~/src/ui/composables/useQuickPublishToast.ts"
...
export interface QuickPublishPlatformItem {
  platformKey: string
  platformName: string
  platformIcon?: string
...
type PublishStatus =
  ...
  | "failed"
  | "preview_ready"
type PublishAction = "" | "publish" | "update" | "delete" | "preview"

export const useQuickPublish = () => {
  ...
  const state = reactive({
    ...
    docTitle: "",
    hasDocument: false,
    platformItems: [] as QuickPublishPlatformItem[],
    activePlatformKey: "",
    activePlatformName: "",
  ...
  const setActivePlatform = (item: QuickPublishPlatformItem) => {
  ...
  const resolvePreviewUrl = async (platformKey: string) => {
    ...
    if (!publishCfg?.cfg) {
      throw new Error(t("quickPublish.error.previewConfigMissing"))
    }
  ...
  const init = async (overridePageId?: string) => {
    ...
    try {
      ...
      const platformItems: QuickPublishPlatformItem[] = []
      ...
      for (const item of enabledConfigArray) {
        ...
        const validation = isAuthorized
          ...
          : { canPublish: false, reason: t("quickPublish.tooltip.unauthorized") }
        ...
        platformItems.push({
          ...
          tooltipText: isPublishReady ? "" : validation.reason || t("publishValidation.incomplete"),
          displayOrder: item.displayOrder,
          isPublishReady,
        })
      }

      state.platformItems = sortQuickPublish(platformItems)

      if (state.hasDocument) {
        try {
          const postInfo = await kernelApi.getBlockByID(pageId)
          const rawTitle = postInfo?.content ?? t("quickPublish.docTitle.untitled")
          state.docTitle = pref.value.fixTitle ? HtmlUtil.removeTitleNumber(rawTitle).replace(/\.md/g, "") : rawTitle
        } catch (e) {
          state.docTitle = t("quickPublish.docTitle.untitled")
        }
      } else {
        state.docTitle = t("quickPublish.docTitle.notDetected")
      }
    } catch (e) {
      state.docTitle = t("quickPublish.docTitle.notDetected")
  ...
  const publishToPlatform = async (item: QuickPublishPlatformItem) => {
    ...
    try {
      ...
      if (!publishCfg?.cfg || !publishCfg?.dynCfg) {
        throw new Error(t("quickPublish.error.publishConfigMissing"))
      }
      const validation = await publishValidation.validatePlatformPublish(item.platformKey)
      if (validation.canPublish !== true) {
        throw new Error(validation.reason || t("publishValidation.incomplete"))
      }
      ...
      if (result?.status) {
        ...
        const warningMsg = sanitizeQuickPublishText(result?.errMsg ?? "")
        const warningDetails = sanitizeQuickPublishText(result?.errDetails || result?.errMsg || "")
        ...
        const errorText = buildQuickPublishErrorText({
          errMsg: result?.errMsg,
          errDetails: result?.errDetails,
          fallback: t("quickPublish.error.publishFailed"),
        })
        setPublishState({
          status: "failed",
      ...
      const errorText = buildQuickPublishCaughtErrorText(error, t("quickPublish.error.publishFailed"))
      setPublishState({
        status: "failed",
        errMsg: errorText.summary,
        errDetails: errorText.details,
  ...
  const previewPlatform = async (item: QuickPublishPlatformItem, openImmediately = false) => {
    ...
    try {
      ...
      if (!previewUrl) {
        throw new Error(t("quickPublish.error.previewUrlMissing"))
      }
      ...
      const errorText = buildQuickPublishCaughtErrorText(error, t("quickPublish.error.previewUrlMissing"))
      setPublishState({
        status: "failed",
        errMsg: errorText.summary,
        errDetails: errorText.details,
  ...
  const deletePlatform = async (item: QuickPublishPlatformItem) => {
    ...
    try {
      const publishCfg = await getPublishCfg(item.platformKey)
      if (!publishCfg?.cfg || !publishCfg?.dynCfg) {
        throw new Error(t("quickPublish.error.deleteConfigMissing"))
      }
      ...
      if (result?.status) {
        ...
        const errorText = buildQuickPublishErrorText({
          errMsg: result?.errMsg,
          errDetails: result?.errDetails,
          fallback: t("quickPublish.error.deleteFailed"),
        })
        setPublishState({
          status: "failed",
          errMsg: errorText.summary,
      ...
      const errorText = buildQuickPublishCaughtErrorText(error, t("quickPublish.error.deleteFailed"))
      setPublishState({
        status: "failed",
        errMsg: errorText.summary,
        errDetails: errorText.details,
   ...
   */
  const forceDeletePlatform = async (item: QuickPublishPlatformItem) => {
    if (!canPublish.value || !item.isAuthorized || !item.isPublished) {
      return
    }
    ...
    try {
      ...
      if (!result) {
        throw new Error(t("quickPublish.error.deleteFailed"))
      }
