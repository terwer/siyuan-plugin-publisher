...
export interface QuickPublishErrorText {
  summary: string
  details: string
}

export interface QuickPublishResultErrorInput {
  errMsg?: unknown
  errDetails?: unknown
  fallback?: string
}
...
export const sanitizeQuickPublishText = (input: unknown): string => {
...
const extractBusinessSummary = (raw: string): string => {
  for (const candidate of buildJsonParseCandidates(raw)) {
    ...
    if (message) {
      return compactSummary(sanitizeQuickPublishText(message))
    }
  }
  ...
  return compactSummary(sanitizeQuickPublishText(fallbackLine || raw))
}
...
const joinDetails = (parts: unknown[], fallback: string): string => {
  ...
  return sanitizeQuickPublishText(detailParts.join("\n\n"))
}

export const buildQuickPublishErrorText = (input: QuickPublishResultErrorInput): QuickPublishErrorText => {
...
export const buildQuickPublishCaughtErrorText = (error: unknown, fallback: string): QuickPublishErrorText => {
  ...
  return buildQuickPublishErrorText({
