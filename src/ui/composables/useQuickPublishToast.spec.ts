...
import { buildQuickPublishToast } from "~/src/ui/composables/useQuickPublishToast.ts"
...
describe("buildQuickPublishToast", () => {
  it("returns null for non-terminal statuses", () => {
    expect(
      buildQuickPublishToast(t, {
        status: "publishing",
        platformName: "语雀",
        lastAction: "publish",
      })
  ...
  it("uses conversational success copy with platform name", () => {
    const payload = buildQuickPublishToast(t, {
      status: "success",
      platformName: "语雀网页版",
      lastAction: "publish",
    })
