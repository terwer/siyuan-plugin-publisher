...
const quickPublishFailedDescription = (
  ...
  if (action === "update") {
    return platformName
      ? t("publish.desc.updateFailedWithReason.named", { name: platformName, reason })
      : t("publish.desc.updateFailedWithReason.default", { reason })
  }
  if (action === "delete") {
    return platformName
      ? t("publish.desc.deleteFailedWithReason.named", { name: platformName, reason })
      : t("publish.desc.deleteFailedWithReason.default", { reason })
  }
  return platformName
    ? t("publish.desc.publishFailedWithReason.named", { name: platformName, reason })
    : t("publish.desc.publishFailedWithReason.default", { reason })
}

describe("quick publish failure feedback copy", () => {
