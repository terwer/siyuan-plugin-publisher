  ...
  <div v-if="isCookieWebPlatform" class="syp-web-cookie-auth" :class="`is-${statusType}`">
    <div class="syp-web-cookie-auth__header">
      <div class="syp-web-cookie-auth__identity">
        <span class="syp-web-cookie-auth__title">{{ t("webCookieAuth.title") }}</span>
    ...
    <div class="syp-web-cookie-auth__body">
      ...
      <div class="syp-web-cookie-auth__actions">
        ...
        >
          ...
          {{ t("webCookieAuth.action.openLogin") }}
        </SypTooltip>
        <SypTooltip
          tag="button"
          type="button"
        ...
        >
          ...
          {{ isLoading ? t("webCookieAuth.action.reading") : t("webCookieAuth.action.autoRead") }}
        </SypTooltip>
        <SypTooltip
          v-if="canLogout"
          tag="button"
          ...
          @click="handleLogout"
        >
          {{ isLogoutLoading ? t("webCookieAuth.action.loggingOut") : t("webCookieAuth.action.logout") }}
        </SypTooltip>
      </div>
    </div>
  </div>
...
const statusLabel = computed(() => {
  if (lastStatus.value === "success") {
    return t("webCookieAuth.status.success")
  }
  if (needsSiteUrl.value) {
    return t("webCookieAuth.status.needSiteUrl")
  }
  if (!isAutoCaptureSupportedByHost.value) {
    return t("webCookieAuth.status.manual")
  }
  if (["no_cookie", "validation_failed", "error"].includes(lastStatus.value)) {
    return t("webCookieAuth.status.retry")
  }
  return props.dynCfg?.isAuth ? t("webCookieAuth.status.authorized") : t("webCookieAuth.status.ready")
})

const statusDescription = computed(() => {
  if (needsSiteUrl.value) {
    return t("webCookieAuth.desc.needSiteUrl")
  }
  if (!isAutoCaptureSupportedByHost.value) {
    return t("webCookieAuth.desc.unsupported")
  }
  if (lastStatus.value === "success") {
    return t("webCookieAuth.desc.success")
  }
  if (lastStatus.value === "no_cookie") {
    return t("webCookieAuth.desc.noCookie")
  }
  if (lastStatus.value === "validation_failed") {
    return t("webCookieAuth.desc.validationFailed")
  }
  if (lastStatus.value === "error") {
    return t("webCookieAuth.desc.error")
  }
  return t("webCookieAuth.desc.ready")
})

const autoCaptureTooltip = computed(() => {
  if (needsSiteUrl.value) {
    return t("webCookieAuth.tooltip.needSiteUrl")
  }
  if (isAutoCaptureSupportedByHost.value) {
    return t("webCookieAuth.tooltip.autoRead")
  }
  return t("webCookieAuth.tooltip.unsupported")
})
const openLoginTooltip = computed(() => {
  ...
  if (isAutoCaptureSupportedByHost.value) {
    return t("webCookieAuth.tooltip.openLogin")
  }
  return t("webCookieAuth.tooltip.unsupported")
})
const manualLabel = computed(() => {
  return props.isManualExpanded ? t("webCookieAuth.manual.expanded") : t("webCookieAuth.manual.collapsed")
})
const manualTooltip = computed(() => {
  return props.isManualExpanded ? t("webCookieAuth.tooltip.manualExpanded") : t("webCookieAuth.tooltip.manual")
})
const logoutTooltip = computed(() => {
  return t("webCookieAuth.tooltip.logout")
})

const messageByStatus = (status: WebCookieAuthorizationStatus) => {
  switch (status) {
    case "success":
      return t("webCookieAuth.message.success")
    case "no_cookie":
      return t("webCookieAuth.message.noCookie")
    case "validation_failed":
      return t("webCookieAuth.message.validationFailed")
    case "unsupported":
      return t("webCookieAuth.message.unsupported")
    case "not_cookie_platform":
      return t("webCookieAuth.message.notCookiePlatform")
    case "platform_not_found":
      return t("webCookieAuth.message.platformNotFound")
    default:
      return t("webCookieAuth.message.error")
  }
}
...
const logoutMessageByStatus = (status: WebCookieLogoutStatus) => {
  switch (status) {
    case "logout_success":
      return t("webCookieAuth.message.logoutSuccess")
    case "url_fallback":
      return t("webCookieAuth.message.logoutFallback")
    case "platform_not_found":
      return t("webCookieAuth.message.platformNotFound")
    case "not_cookie_platform":
      return t("webCookieAuth.message.notCookiePlatform")
    case "no_logout_method":
      return t("webCookieAuth.message.noLogoutMethod")
    case "persist_failed":
      return t("webCookieAuth.message.logoutPersistFailed")
    case "logout_failed":
      return t("webCookieAuth.message.logoutFailed")
    default:
      return t("webCookieAuth.message.logoutFailed")
  }
}
...
async function handleAutoCapture() {
  ...
  if (!isAutoCaptureReady.value) {
    if (!isAutoCaptureSupportedByHost.value) {
      ...
      MessageWarning(messageByStatus("unsupported"))
      emit("authorized", { status: "unsupported", ok: false })
    } else if (!hasResolvableAuthUrl.value) {
      MessageWarning(t("webCookieAuth.message.needSiteUrl"))
    }
    return
  }
  ...
  try {
    ...
    if (result.ok) {
      MessageSuccess(messageByStatus(result.status))
    } else {
      MessageWarning(messageByStatus(result.status))
    }
    emit("authorized", { status: result.status, ok: result.ok })
  } catch {
    lastStatus.value = "error"
    MessageError(messageByStatus("error"))
...
async function handleLogout() {
  ...
  try {
    await ElMessageBox.confirm(t("webCookieAuth.confirm.logout.message"), t("webCookieAuth.confirm.logout.title"), {
      type: "warning",
  ...
  try {
    ...
    if (result.status === "logout_success") {
      ...
      MessageWarning(logoutMessageByStatus(result.status))
    } else {
      MessageError(logoutMessageByStatus(result.status))
    }
    emit("authorized", { status: result.status, ok: result.ok })
  } catch {
    MessageError(logoutMessageByStatus("logout_failed"))
