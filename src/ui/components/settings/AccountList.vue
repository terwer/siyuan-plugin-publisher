<template>
  <section class="syp-settings-page">
    <div class="syp-settings-page__header">
      <div>
        <div class="syp-settings-page__eyebrow">{{ t("account.eyebrow") }}</div>
        <h2 class="syp-settings-page__title">{{ t("account.title") }}</h2>
        <p class="syp-settings-page__desc">{{ t("account.desc") }}</p>
      </div>
      <button type="button" class="syp-btn syp-btn-primary" @click="$emit('add')">
        {{ t("account.action.add") }}
      </button>
    </div>

    <div v-if="items.length === 0" class="syp-settings-empty">
      <div class="syp-settings-empty__title">{{ t("account.empty.title") }}</div>
      <div class="syp-settings-empty__desc">{{ t("account.empty.desc") }}</div>
    </div>
    ...
    <div v-else class="syp-account-list">
      ...
      >
        <div class="syp-account-item__order">
          <SypTooltip
            tag="button"
            :content="t('account.order.dragHandle')"
            inline-flex
            type="button"
            class="syp-account-item__drag-handle"
            draggable="true"
            :aria-label="t('account.order.dragHandle')"
          ...
          <div class="syp-account-item__order-buttons">
            <SypTooltip
              tag="button"
              :content="t('account.order.moveUp')"
              inline-flex
              type="button"
              class="syp-account-item__order-button"
              :disabled="index === 0"
              :aria-label="t('account.order.moveUp')"
            ...
            <SypTooltip
              tag="button"
              :content="t('account.order.moveDown')"
              inline-flex
              type="button"
              class="syp-account-item__order-button"
              :disabled="index === orderedItems.length - 1"
              :aria-label="t('account.order.moveDown')"
              @click="moveItem(item.platformKey, 1)"
            >
              ↓
            </SypTooltip>
        ...
        <div class="syp-account-item__actions">
          ...
          >
            {{ item.isAuth ? t("account.action.manage") : t("account.action.authorize") }}
          </button>

          <div class="syp-account-item__toggle">
            <span class="syp-account-item__toggle-label">
              {{ item.isEnabled ? t("account.toggle.enabled") : t("account.toggle.disabled") }}
            </span>
            <SypTooltip
              tag="label"
              :content="item.isEnabled ? t('account.toggle.disableHint') : t('account.toggle.enableHint')"
              inline-flex
            ...
            >
              <input
                ...
                :aria-label="item.isEnabled ? t('account.toggle.disable') : t('account.toggle.enable')"
          ...
          <SypTooltip
            ...
            tag="button"
            :content="t('account.action.delete')"
            inline-flex
            type="button"
...
import SypConfirmBar from "~/src/ui/components/common/SypConfirmBar.vue"
import SypTooltip from "~/src/ui/components/common/SypTooltip.vue"
import { useAppI18n } from "~/src/ui/composables/useAppI18n.ts"
import type { AccountItem } from "~/src/ui/composables/useSettings.ts"
...
const orderedItems = computed(() => {
  ...
  const knownItems = localPlatformKeys.value.map((key) => itemMap.get(key)).filter(Boolean) as AccountItem[]
...
function deleteConfirmMessage(platformName: string, platformKey: string) {
  ...
  return `${t("account.action.deleteConfirmTitle")} ${t("account.action.deleteConfirmText", { name })}`
}
