<template>
  <section class="syp-single-view" role="region" :aria-label="t('panel.singlePublish')">
    <div class="syp-single-view__head">
      <button
        v-if="!embedded"
        type="button"
        class="syp-single-view__back"
        :aria-label="t('singlePublish.back')"
        @click="goBack"
      >
        <LucideChevronLeft />
      </button>
      <div>
        <div class="syp-single-view__eyebrow">{{ t("panel.singlePublish") }}</div>
...
const headTitle = computed(() => {
  return selectedKey.value ? t("panel.singlePublish") : t("singlePublish.selectPlatform.title")
})
