<template>
  <section class="syp-quick-grid" role="region" :aria-label="t('panel.quickPublish')">
    ...
    <div v-else-if="!hasPlatforms" class="syp-quick-grid__empty">
      <p class="syp-quick-grid__empty-text">{{ t("singlePublish.empty.noPlatform") }}</p>
    </div>
    ...
    <div class="syp-quick-grid__hint">{{ t("singlePublish.status.previewHint") }}</div>
  </section>
</template>
