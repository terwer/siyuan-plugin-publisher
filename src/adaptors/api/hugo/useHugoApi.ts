...
const useHugoApi = async (key: string, newCfg?: HugoConfig) => {
  // 创建应用日志记录器
  const logger = createAppLogger("use-hugo-api")
  ...
  if (newCfg) {
    ...
    if (ObjectUtil.isEmptyObject(cfg)) {
      ...
      cfg = new HugoConfig(githubUsername, githubAuthToken, githubRepo, githubBranch, middlewareUrl)
  ...
  cfg.placeholder.knowledgeSpaceReadonlyModeTip = "Hugo 平台暂不支持修改发布目录，如需修改，请删除之后重新发布"
  cfg.knowledgeSpaceType = CategoryTypeEnum.CategoryType_Tree_Single

  // 创建 Hugo 的 yamlAdaptor
  const yamlAdaptor = new HugoYamlConverterAdaptor()

  // 创建 Hexo API 适配器
  const blogApi = new HugoApiAdaptor(appInstance, cfg)
  logger.info("Hugo API created successfully.", cfg)
