...
- **Img ✅**：图床切「**当前平台**」后重发 → cat 图上传 `source/images/cat-20260822153711-o2ho0mg.jpg`（13246B），.md 引用改写相对路径 `![cat](../images/cat-...jpg)`（当前方案，源码+构建产物双模式）。
...
4. **图片引用采用相对路径 `../images`（源码+构建产物双模式，2026-08-26）**：**关键约束**（本地 `hexo-blog` 核查）：`permalink:/post/:title.html` 或插件写入的 `permalink:/post/{slug}.html` 都让发布时间 URL 恒定位于站点根下 1 层，这是 `../images/` 在构建后仍能到达 `/images/` 的前提；`post_asset_folder:false` 使每篇相对资产不适用。**验证结论**（本地 `hexo generate` + `hexo server`）：相对 `../images/…jpg` → 产物 `<img src="/../images/…jpg">`，服务端/浏览器把 `/../` 归一化为 `/images/…`（HTTP 200）；源码 `source/_posts/x.md` 的 `../images/` 解析到 `source/images/…`（文件存在）——源码与构建产物均可用。**实现**：`HexoConfig.imageLinkPath = "../images"`（`imageStorePath` 保持 `source/images`）；`CommonGithubApiAdaptor.getImagePath` 支持 `../`/`./` 相对前缀、拼接文件名、不前置站点根斜杠。用 `HexoApiAdaptor.newMediaObject` 单测断言 `attachment.url==='../images/{name}'`、提交目录 `source/images/{name}`。**依赖 `yamlLinkEnabled` 默认 `true`**（写入 `permalink:/post/{slug}.html`，平铺 URL 是 `../images/` 生效前提）；关掉时 `_config.yml` 的 `/post/:title.html` 也是 1 层。**存量账号**：需在配置页将「图片访问链接」改为 `../images` 后重发（默认仅对新账号生效）。
