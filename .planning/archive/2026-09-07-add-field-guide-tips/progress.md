...
- **问题**：现界面下鉴权行渲染的是 `V2WebCookieAuthPanel`（去登录/自动读取），手动文本框默认折叠；旧界面的 `field-guide` 只包文本框 → **折叠态该行没有 ⓘ**，违反「每行真实渲染的行都要有指引」。
...
- **不变量**：展开/收起手动编辑时该行 ⓘ **恒为 1**（不出现两个）；旧界面的 `CookieSetting.vue` 未动（旧界面文案与结构零变化）。
...
## 会话：2026-09-12（第十轮：F.3 前置核验——旧界面/locale 零改动取证 + 旧界面打包复核）

### 做了什么
1. **变更面取证**：本 change 全部提交的 diff（`c88f930b^..HEAD`，排除 planning/openspec/docs/tmp）**恰好 21 个文件**，全部落在界面帮助/字段指引层（FieldGuide、helpPageIdKey、4 个设置组件、V2PlatformConfigBridge、14 个 help 配置、registry.spec）。
2. **locales 零改动**：`-- src/i18n src/locales "*.json"` → 空；**旧界面零改动**：旧界面表单 `base/CookieSetting.vue` 与旧界面入口 `Admin.vue` 均不在变更清单 → F.3 的「locales 共享串不动，旧界面文案零变化」由口头承诺变为 git 可核验证据。
3. **旧界面打包链路复核**：`python scripts/build.py`（= `pnpm build`）exit 0，产出 `siyuan-plugin-publisher-1.41.1.zip` → 共用组件挂指引后旧界面构建未受影响；`build/` 已在 `.gitignore:34`，工作树保持干净。
...
- **本次查实的关键硬约束（先前未记录）**：平台 `singleplatform/*/*Setting.vue` 是 **界面各处共用同一批文件** ——
  `routeConfig.ts:32/131-134` 的 `setting-platform-single` → `SingleSettingIndex.vue` 属 **旧界面 app**（`src/main.ts` → `index.html`）；
  现界面侧 `V2PlatformConfigBridge.vue` → `bridgeRegistry.ts` import 的是**同一批**组件，两边都经 `CommonBlogSetting.vue` 读
  `props.cfg.placeholder.*Placeholder`（`:382/390/398/415/434/487`）。
  → **不能直接改组件里的 placeholder 赋值**（会让旧界面文案跟着变，违反 4.2 与 proposal 的「旧界面文本零变化」）。
- **现界面专属信号已存在**：`CommonBlogSetting.vue:59` 已 `inject(V2_PLATFORM_CONFIG_ACTION_BRIDGE_KEY, null)`，现界面有 provider
  （`V2PlatformConfigBridge.vue:120`）、旧界面得 `null` → 现成的「仅在现界面生效」开关，无需新造机制。
...
- 已备三种口径：**A（推荐，改现界面分支的 placeholder 绑定 + `fields` 增可选 `sample`，旧界面零变化）**、
  B（34 个组件各自按现界面信号注入，低侵入但重复）、C（直接改 locales，破坏 4.2 不可接受）。
...
- **目标与口径**：界面配置表单的占位符原本是**长说明**（如「存储目录例如：docs，部分平台可使用 `[auto]` 作为特殊占位符」）。这类文案一开始输入就消失，等于把说明放在最留不住信息的位置；而同一字段的含义已由行尾 ⓘ（`fields.tip`）讲清楚。故界面占位符只给**可直接照抄的示例值**，说明归 ⓘ，两者不再互相重复。
- **机制（含旧界面零变化的关键）**：新增 `src/composables/useFieldPlaceholder.ts`。有 `pageId` 注入时取 `fields.placeholder`，否则**原样返回调用方文案**。`pageId` **仅**由现界面的 `V2PlatformConfigBridge` 提供，旧界面走 standalone 路由无 provider → **旧界面文案零变化**，不必在两个入口间做分支判断。
...
- **单测**：新增 `src/composables/useFieldPlaceholder.spec.ts` 钉住四条边界——旧界面无注入回退原文案 / 界面取示例值 / 界面该字段无示例值回退 / 未登记字段回退（**不产生空串**）。
