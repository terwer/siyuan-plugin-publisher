...
- [ ] F.3 界面表单 placeholder 收敛为示例值（每次改动内保证对应 `fields` 已有说明，不留空窗）；locales 共享串不动，旧界面文案零变化。
  - **「旧界面文案零变化 / locales 不动」已取证（2026-09-12）**：本 change 全部提交的 diff 恰好 21 个文件且全在界面帮助层；`src/i18n`/`*.json` 零改动；旧界面表单 `base/CookieSetting.vue` 与旧界面入口 `Admin.vue` 均不在变更清单；`python scripts/build.py`（旧界面打包）仍成功产出 zip。
...
**平台 `*Setting.vue` 是界面各处共用的同一批文件**：
`routeConfig.ts:32/131-134`（`setting-platform-single` → `SingleSettingIndex.vue`）是 **旧界面 app（`src/main.ts` → `index.html`）** 的入口；
**现界面侧**（`V2PlatformConfigBridge.vue` → `bridgeRegistry.ts`）import 的是**同一批** `singleplatform/*/*Setting.vue`。
两边都经过 `CommonBlogSetting.vue` 读取 `props.cfg.placeholder.*Placeholder`（`CommonBlogSetting.vue:382/390/398/415/434/487`）。
→ **结论：不能直接改这些组件里的 placeholder 赋值**，否则旧界面文案跟着变，违反 change 4.2（「locales 的 `setting.blog.*.tip` 保持不动、旧界面文案零变化」）与 proposal「旧界面文本零变化」承诺。
→ **可用的现界面专属信号已存在**：`CommonBlogSetting.vue:59` 已 `inject(V2_PLATFORM_CONFIG_ACTION_BRIDGE_KEY, null)`——**现界面才有 provider**（`V2PlatformConfigBridge.vue:120`）、旧界面取到 `null`。这就是「仅在现界面生效」的现成开关，无需新造机制。
...
| 方案 | 做法 | 旧界面影响 | 成本/风险 |
|---|---|---|---|
| **A（推荐）** | 不改 locales、不改共用组件赋值；在 `FieldGuide` 体系内新增「示例值」来源（`fields` 增可选 `sample?: string`），由 `CommonBlogSetting.vue` 在 **现界面分支**（`v2ActionBridge != null`）把 `:placeholder` 从 `placeholder.*Placeholder` 切到 `sample` | **零变化**（旧界面走 `v2ActionBridge === null` 分支，仍读 locales） | 中：动 `FieldGuide` + 共用表单的 placeholder 绑定；需尺子钉死「旧界面分支不变」 |
| **B** | 各平台 `*Setting.vue` 按现界面信号（inject 到 bridge key）选择性地注入示例值到自己的 `*Placeholder` 实例 | 零变化 | 低侵入，但 34 个组件各写一遍判断 → 重复 |
| **C** | 直接改 locales 共享串为示例值 | **破坏 4.2/提案承诺**（旧界面文案变） | 不可接受（除非用户明确同意放弃 4.2） |
...
| Cookie 行（现界面授权面板与手动文本框共存） | cookie | 面板按钮写 `cfg.password`，手动框绑 `cfg.password` | `password`（**整行只挂一条**，面板与文本框共用，勿各挂一个） |
...
## 备注
- 验证战役暂停在 `.planning/platform-verification/`（T1 已 22/35 ✅，下一步 #12 Vitepress，目标仓库已查实 `terwer/siyuan-developer-docs`）。本任务全部验收后回到 #12。
