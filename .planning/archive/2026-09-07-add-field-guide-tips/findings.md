...
- 排序原则：最大扩展性 > 可用性 > 维护成本；功能 100% 保留（旧界面文案与行为不动）。
...
- **鉴权行「一条指引服务两个控件」**：现界面下该行渲染 `V2WebCookieAuthPanel`（去登录 / 自动读取 Cookie / 退出），手动文本框默认折叠。旧界面的实现把 `field-guide` 只包文本框 → 折叠态该行**没有 ⓘ**。定稿改法：`<field-guide field="password" tall>` 包住整行内容（新增 `.cookie-form-item__body` 容器：插槽面板 + 折叠文本框 + 提示），展开/收起 ⓘ 恒为 1。**这是 Cookie 族 8 站共用的挂载口径**，后续 7 站不再重新讨论。
...
## 第十轮：F.3 前置核验（旧界面/locale 零改动证据）+ 单一 pageId 拼接核验（2026-09-12）
- **变更面收敛证明**：`git diff --stat c88f930b^..HEAD`（本 change 的全部提交，排除 planning/openspec/docs/tmp）**恰好 21 个文件**，全部落在界面帮助/字段指引层：
...
- **旧界面零改动**：旧界面表单 `base/CookieSetting.vue`、旧界面入口 `Admin.vue` → **不在变更清单内**（清单仅 21 个文件，已全列）→「旧界面文案零变化」有 git 证据。
- **旧界面打包链路仍通过**：`python scripts/build.py`（= `pnpm build`）成功产出 `siyuan-plugin-publisher-1.41.1.zip`（exit 0）→ 共用组件挂载字段指引后旧界面构建/打包未受影响（`build/` 已在 `.gitignore:34`，工作树保持干净）。
...
## 第十二轮：SOP §3 宿主判定全部脚本化（两条命令覆盖「指引 + 帮助引导」）（2026-09-12）
- **新增 `tmp/host-help-gate-check.ps1` + `.js`**：覆盖 SOP §3 的「帮助引导与文档」环节——帮助入口可打开面板、面板为**平台专属**（有 summary 与完整文档入口、**不出现**「暂无专属帮助文档」回退）、FAQ 条数、引导入口存在、点击引导后**逐步**确认 `.syp-tour-overlay__highlight` 可见且尺寸非零、**不出现** `--missing`、引导能正常收尾、面板与引导层在 `.syp-v2` 内且未被祖先裁切；退出码即结论。全程只读（打开面板 → 跑引导 → 关面板），不改配置不保存。
- **语雀网页版（#27）实跑全绿**：面板标题「语雀网页版」、在 `.syp-v2` 内 ✓、视口内 ✓、未被裁切 ✓；summary 43 字、文档入口 8 字、FAQ **3** 条、**无回退提示** ✓；引导 **4/4 步命中真实控件**，高亮尺寸 739.2×95.2 / 739.2×48 / 739.2×48 / 739.2×74.2 —— 与第五轮手工量取的数字**完全一致**（互为独立复现）；引导正常收尾 → **exit 0**。
- **失败路径也验证过**：用一份构造的负例报告跑同一包装脚本，六项未通过被逐条列出并 `exit 1` ✓ —— 说明这把尺不是「只会绿的尺」。
- **两条宿主尺连跑**（当前构建、语雀网页版页）：`host-field-guide-check` → exit 0、`host-help-gate-check` → exit 0；跑完宿主无残留（帮助面板 0、引导层 0），未改动任何配置。
- **SOP §3 草稿已补入工具行**：把这两条命令写进「字段指引必须真实渲染并可核验」项的核验方式（含 `-AllowNoGuide` 豁免声明的用法），使该必过项从「人工逐行核对」变为「一条命令复现 + 退出码判定」。

## 第十三轮：宿主尺自查发现并修掉「假绿」+ #28 预制件（2026-09-12）
- **拿已知坏页（#28 Halo网页版）试尺，抓到尺子自身的假绿**：原判定用 `.syp-field-guide` **包装器**计数，而「键名写错/漏键」时包装器仍在、只是不渲染图标 → 会把「6 行 6 包装器」判成通过，恰好漏掉 D 组这类缺陷。已改为**只认 `.syp-field-guide__icon[data-syp-field-guide]`**（真实渲染的指引），并新增 `emptyWrappers` 判定「已挂载但未渲染指引（键名/键缺失）」；失败路径也顺带修掉一处 `icon` 为 null 时的空指针（会导致整份报告为空）。
