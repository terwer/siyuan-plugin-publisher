...
Publisher 的 HelpPanel、FieldGuide、TourGuide 框架已经通过 `platform-help-guide-system` 上线并归档，但平台配置帮助覆盖仍不均衡。博客园 `metaweblog_Cnblogs` 已有完整 `summary / fields / faq / tour`，其余大量 T1 平台仍集中在 `platform-config/_default.ts`，只有 `helpUrl` 级别的兜底。
...
- 轻量兜底清单：`src/helpConfigs/pages/platform-config/_default.ts`
...
- 不把帮助覆盖状态混入 `platform-checklist.md` 的 Cfg/Pub/Upd/Del/Img 主表。
...
4. 对仍在 `platform-config/_default.ts` 的已验证平台，拆出独立 config 文件并注册。
...
- `src/helpConfigs/pages/platform-config/_default.ts`
...
- 从 `platform-config/_default.ts` 拆出本地系统、知乎、CSDN 独立配置。

### Phase 2：#28 HaloWeb 跟进

- 等 `platform-verification` 中 #28 完成 Cfg/Pub/Upd/Del/Img。
...
- 回滚：帮助配置是纯叠加，可移回 `platform-config/_default.ts` 或删除新增独立配置，不影响发布链路。

## 9. 待确认问题

- 明天第一批是否按“已验证但未完整覆盖”优先，即本地系统、知乎、CSDN、语雀 API、WordPress、语雀网页版？默认如此。
- #28 HaloWeb 是否要等完整全链路验证后再补，还是先补 Cfg 配置页帮助？默认等手验结果更完整后补，但 Cfg 的已知坑点可以先补 FAQ。
